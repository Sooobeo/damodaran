"""TranslateGemma v2: source-only CPU comparison with explicit EOG/memory evidence."""
...
def memory_requirements(model_size, model):
    ...
    return {"weightRepackingEnabled": False, "estimatedKvCacheUpperBoundBytes": kv_bytes,
            ...
            "requiredAvailablePhysicalBytes": model["size"] + 2 * GIB,
            ...
            "allocationGuarantee": False, "continuousMemoryMonitor": False}
...
def server_command(dest, model, port, key, threads):
    return [str(dest / "runtime/llama-server.exe"), "--model", str(dest / model["name"]),
            ...
            "--gpu-layers", "0", "--device", "none", "--no-op-offload", "--no-repack", "--threads", str(threads),
...
def run(args):
    ...
    summary = {"version": "translategemma-large-screen-v2", "status": "running", "startedAt": datetime.now(timezone.utc).isoformat(),
    ...
    try:
        ...
        policy = memory_requirements(args.model_size, model)
