"""Source-free Windows fixture: 256 MiB file mapping under a 128 MiB WS cap.
...
class _ProcessOnlyDiagnostic:
    ...
    def __init__(self, owner):
        self.owner, self.api = owner, _Native()
        self.identity = None
        self.job_record = {"workingSetLimitApplied": False, "killOnCloseOwnedJobOnly": True}
    ...
    def handle(self, child):
        ...
        if child.poll() is not None or self.api.kernel.GetProcessId(handle) != child.pid:
            raise RuntimeError("process_only_fixture_identity_changed")
        return handle

    def checked_limits(self, handle):
        limits = self.api.query_process(handle)
        if limits["minimumBytes"] != 1024**2 or limits["maximumBytes"] != MAXIMUM or limits["flags"] != 6:
            raise RuntimeError("process_only_fixture_readback_mismatch")
        return limits

    def apply_child(self, child):
        handle = self.handle(child)
        before = self.api.query_process(handle)
        if not self.api.kernel.SetProcessWorkingSetSizeEx(handle, 1024**2, MAXIMUM, 6):
            self.api.fail("process_only_fixture_set_failed")
        after = self.checked_limits(handle)
        self.identity = (child.pid, handle)
        return {"pid": child.pid, "before": before, "after": after,
                "jobLimitAppliedBeforeSpawn": False, "processFlagsAppliedAfterSpawn": True,
                "fixtureWaitsForStdinBeforeMapping": True}

    def sample_child(self, child):
        handle = self.handle(child)
        if self.identity != (child.pid, handle):
            raise RuntimeError("process_only_fixture_handle_changed")
        limits = self.checked_limits(handle)
        counters = _MemoryCounters()
        counters.cb = ctypes.sizeof(counters)
        if not self.api.psapi.GetProcessMemoryInfo(handle, ctypes.byref(counters), ctypes.sizeof(counters)):
            self.api.fail("process_only_fixture_memory_failed")
        return {"pid": child.pid, "monotonicSeconds": time.monotonic(),
                "workingSetBytes": int(counters.WorkingSetSize),
                "peakWorkingSetBytes": int(counters.PeakWorkingSetSize),
                "privateBytes": int(counters.PrivateUsage), "pageFaultCount": int(counters.PageFaultCount),
                "workingSetLimits": limits}


def fixture(directory, process_only=False):
    summary = {"version": "working-set-native-fixture-v1", "status": "failed",
    ...
    try:
        ...
        if summary["maximumObservedWorkingSetBytes"] > MAXIMUM:
            raise RuntimeError("fixture_observed_working_set_above_limit")
...
def main():
    ...
    summary = {"version": "working-set-fixture-verification-v1", "status": "failed",
    ...
    try:
        ...
        for relative in ("working_set_limit.py", "test_working_set_limit.py"):
