...
import ledger
from relation_checks_v2 import inspect_relations, TYPES, VERSION
...
EXTRA_FIXTURE = "content/model-comparison/input-preparation-v1-s2s3/relation-fixtures-v2-extra.json"
EXTRA_FREEZE = "content/model-comparison/input-preparation-v1-s2s3/relation-fixtures-freeze-v2-extra.json"
REVIEW_FIXTURE = "content/model-comparison/input-preparation-v1-s2s3/relation-fixtures-v2-review.json"
REVIEW_FREEZE = "content/model-comparison/input-preparation-v1-s2s3/relation-fixtures-freeze-v2-review.json"
SELECTED = {
...
def prepare(output):
    ...
    extra=ev.json(EXTRA_FIXTURE); extra_receipt=ev.json(EXTRA_FREEZE)
    ledger.require(ev.files[EXTRA_FIXTURE]==extra_receipt["fixtureSha256"] and len(extra["cases"])==extra_receipt["caseCount"],"extra_fixture_freeze_changed")
    reviewed=ev.json(REVIEW_FIXTURE); reviewed_receipt=ev.json(REVIEW_FREEZE)
    ledger.require(ev.files[REVIEW_FIXTURE]==reviewed_receipt["fixtureSha256"] and len(reviewed["cases"])==reviewed_receipt["caseCount"],"review_fixture_freeze_changed")
    for name in ("relation_checks_v2.py","test_relation_checks_v2.py","test_relation_diagnostic.py","run_relation_diagnostic_v2.py"):
        ev.read(Path(__file__).with_name(name))
    ev.read("scripts/model-comparison/error-ledger/ledger.py")
    # Preserve the unsuccessful v1 diagnostic as predecessor evidence.
    predecessor=BASE/"s3-relations-20260913-v1"
    previous=ev.json(predecessor/"relation-manifest.json")
    ev.read(predecessor/"relation-report.json")
    ev.read(predecessor/"relation-diagnostics.json")
    for path,digest in previous["evidenceFiles"].items():ev.read(path,digest)
...
def run(output, append=False):
    ...
    fixtures["cases"]+=ev.json(EXTRA_FIXTURE,manifest["evidenceFiles"][EXTRA_FIXTURE])["cases"]
    fixtures["cases"]+=ev.json(REVIEW_FIXTURE,manifest["evidenceFiles"][REVIEW_FIXTURE])["cases"]
    synthetic=[]
    ...
    for row in inputs:
        events.append(dict(version=ledger.VERSION,kind="relation_observation",checker=VERSION,
                           checkerCodeSha256=manifest["evidenceFiles"][Path(__file__).with_name("relation_checks_v2.py").relative_to(ROOT).as_posix()],
