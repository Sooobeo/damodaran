...
VERSION = "input-preparation-relations-v2"
...
UNIT = r"(?:percentage\s+points?|percent(?:age)?|%\s*포인트|%\s*[pP]|퍼센트\s*포인트|퍼센트|%|dollars?|달러|euros?|유로|centimeters?|centimetres?|센티미터|cm|meters?|metres?|미터|m)"
QUANTITY = re.compile(rf"(?<![\w.])(?:(?P<prefix>[$€])\s*(?P<pn>{NUMBER})|(?P<n>{NUMBER})\s*(?P<unit>{UNIT})(?![A-Za-z]))", re.I)
...
def unit_name(value: str) -> str:
    ...
    if value in ("€", "euro", "euros", "유로"):
        return "EUR"
    if value in ("percentagepoint", "percentagepoints", "%p", "%포인트", "퍼센트포인트"):
...
def change_relation(source, target, kind):
    ...
    if re.match(r"\s*(?:수준\s*으로|으로|로|까지)", tail):
        role_b = "result"
    elif re.match(r"\s*(?:만큼|(?:가|는|은).*증가분|(?:를|을)?\s*(?:증가|높이|높인|높였|올리|올린|올렸|오른|올랐|상승|감소|낮추|낮춘|내리|내린|떨어|줄어))", tail):
        role_b = "delta"
    else:
        role_b = "undetermined"
    if kind == "percent_change":
        ...
        if role_b == "undetermined":
            return result(kind, "undetermined", "수치와 변화 술어 사이의 조사가 명시 증분/결과 패턴에 맞지 않는다.", source, target, [ss], [ts])
        agree = role_a == role_b
...
def inclusion_negation(source, target):
    ...
    m = matches[0]; ss = sentence(source, m.start(), m.end())
    if len(re.findall(r"\b(?:includ(?:e|es)|exclud(?:e|es))\b", source[ss[0]:ss[1]], re.I)) != 1:
        return result(kind, "undetermined", "동일 절의 다른 포함/제외 대상을 단일 대상에 연결하지 않는다.", source, target, [ss])
    ...
    if not verb or len(re.findall(r"포함|제외",clause)) != 1 or len(re.findall(r"않|아니",clause)) > 1 or re.search(r"수\s*있|수\s*없|것(?:은|이)\s*아니", clause):
