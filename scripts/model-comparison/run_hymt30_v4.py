"""V4 isolated Hy-MT2-30B-A3B Q4 CPU screen; validate native top-k normalization.
...
VERSION = "hymt30-development-screen-v4"
...
# b10888 server field_num clamps top_k=-1 to its nonnegative lower bound.
# Its CPU sampler constructs the same empty sampler for every k <= 0.
# Keep the requested -1 untouched and retain the actually reported integer 0.
SAMPLING_NORMALIZATION = {
    "parameter": "top_k", "requestedValue": -1, "expectedReportedValue": 0,
    "meaning": "disabled", "runtimeRevision": setup.RUNTIME_REVISION,
...
FROZEN.update({common.ROOT / item["path"]: item["sha256"] for item in SAMPLING_NORMALIZATION["sourceEvidence"]})
...
def validate_settings(settings):
    ...
    for k in ("temperature", "top_p", "top_k", "repeat_penalty", "repeat_last_n", "min_p", "seed",
        ...
        if k == "top_k":
            require(SAMPLING[k] == -1 and type(actual) is int and actual == 0,
                    "actual_sampling_mismatch_top_k")
            continue
...
def prediction(row, prompt, matches, tokens, response, elapsed):
    ...
    return {
        ...
        "samplingNormalization": {**SAMPLING_NORMALIZATION,
                                  "actualReportedValue": settings["top_k"], "applied": True},
...
def code_hashes():
    ...
    paths.extend(common.ROOT / item["path"] for item in SAMPLING_NORMALIZATION["sourceEvidence"])
