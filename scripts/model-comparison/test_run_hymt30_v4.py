...
import run_hymt30_v4 as runner


class Hy30V4Tests(unittest.TestCase):
    def test_v3_bytes_preserved_and_only_reported_zero_accepted(self):
        self.assertEqual(runner.common.digest(Path(runner.__file__).with_name("run_hymt30_v3.py")),
                         "d4b76adececad932a27689d3bf35155a941e45bac737962cadff4bcd4b428c70")
        settings = copy.deepcopy(runner.SAMPLING)
        settings["top_k"] = 0
        self.assertEqual(runner.validate_settings(settings)["top_k"], 0)
        self.assertEqual(runner.SAMPLING["top_k"], -1)
        for value in (-1, -2, 1, 20, 0.0, False, None):
            with self.assertRaisesRegex(ValueError, "actual_sampling_mismatch_top_k"):
                runner.validate_settings(settings | {"top_k": value})

    def test_other_sampling_fields_are_not_relaxed(self):
        settings = copy.deepcopy(runner.SAMPLING) | {"top_k": 0}
        for key, value in {"temperature": 0.8, "top_p": 0.6, "repeat_penalty": 1.05,
                           "repeat_last_n": 0, "min_p": 0.1, "seed": 43,
                           "n_predict": 200, "ignore_eos": True, "stop": ["END"],
                           "samplers": ["temperature"], "presence_penalty": 1,
                           "frequency_penalty": 1, "dry_multiplier": 1, "mirostat": 1,
                           "dynatemp_range": 1, "typical_p": 0.5, "xtc_probability": 0.5,
                           "top_n_sigma": 1}.items():
            with self.assertRaisesRegex(ValueError, "actual_sampling_mismatch_" + key):
                runner.validate_settings(settings | {key: value})
    ...
    def test_generation_contract_unchanged(self):
        ...
        self.assertEqual(runner.VERSION, "hymt30-development-screen-v4")
    ...
    def test_raw_output_and_integrity_rules_unchanged(self):
        ...
        expected = previous.prediction(row, "fixture", [], [1], response, 1)
        response["generation_settings"]["top_k"] = 0
        actual = runner.prediction(row, "fixture", [], [1], response, 1)
        expected["actualGenerationSettings"]["top_k"] = 0
        self.assertEqual({k: v for k, v in actual.items() if k != "samplingNormalization"}, expected)
        self.assertEqual(actual["samplingNormalization"]["requestedValue"], -1)
        self.assertEqual(actual["samplingNormalization"]["actualReportedValue"], 0)
        self.assertTrue(actual["samplingNormalization"]["applied"])
