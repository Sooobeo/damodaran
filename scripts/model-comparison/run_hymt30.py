"""Isolated Hy-MT2-30B-A3B Q4 CPU screen; never trains or deploys.
...
VERSION = "hymt30-development-screen-v1"
...
def preflight(available=None):
    if available is None:
        available = screen.memory_status()["availablePhysical"]
    kv = 48 * CONTEXT_SIZE * 4 * 128 * 2 * 2
    headroom = 3 * 1024 ** 3
    return {
        "availablePhysicalBytes": available, "modelBytes": setup.MODEL["size"],
        "f16KvBudgetBytes": kv, "scratchAndHeadroomEstimateBytes": headroom - kv,
        "minimumAvailablePhysicalBytes": setup.MODEL["size"] + headroom,
        "passed": available >= setup.MODEL["size"] + headroom,
        "scratchMeasured": False, "kvFormula": "48 layers * 8192 positions * 4 KV heads * 128 dimensions * 2 K/V * 2 bytes",
        "policy": "all resident Q4 weights + 0.75 GiB KV + 2.25 GiB estimated heap/scratch/headroom",
...
def server_command(port, key):
    return [str(setup.DEST / "runtime-cpu/llama-server.exe"), "--model", str(setup.DEST / setup.MODEL["name"]),
        ...
        "--device", "none", "--no-op-offload", "--load-mode", "mmap", "--fit", "off",
...
def code_hashes():
    paths = [Path(__file__).resolve(), Path(setup.__file__).resolve(), Path(screen.__file__).resolve(), OWNER]
...
def run(args):
    ...
    summary = {
        ...
        "memoryGuard": {"minimumAvailablePhysicalBytes": 512 * 1024 ** 2, "durationSeconds": 3, "sampleIntervalSeconds": 0.5},
    }
    process = log = monitor = None
    stop_monitor = threading.Event()
    memory_samples, records = [], []
    ...
    try:
        ...
        if not args.run:
            ...
            require(summary["preflightBeforeIntegrity"]["passed"], "insufficient_physical_memory")
            ...
            require(summary["preflightImmediatelyBeforeSpawn"]["passed"], "insufficient_physical_memory")
            ...
            def observe_memory():
                ...
                while not stop_monitor.wait(0.5) and process.poll() is None:
                    try:
                        ...
                        if time.monotonic() - load_started >= TOTAL_SECONDS:
                            summary["timeGuardAborted"] = True
                            process.kill()
                            return
                        if state["availablePhysical"] < 512 * 1024 ** 2:
            ...
            with (output / "predictions.jsonl").open("x", encoding="utf-8", newline="\n") as destination:
                for row in rows:
                    ...
                    record.update(rawResponseFile=raw_path.name, rawResponseSha256=common.digest(raw_path),
                                  memoryBefore=before_memory, memoryAfter=memory_state(process))
        ...
        common.write_once(output / "memory-samples.json", memory_samples)
