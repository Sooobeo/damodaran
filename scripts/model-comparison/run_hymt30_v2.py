"""V2 isolated Hy-MT2-30B-A3B Q4 CPU screen with separate commit guards.
...
VERSION = "hymt30-development-screen-v2"
...
def preflight(available=None, available_commit=None):
    ...
    headroom = 3 * 1024 ** 3
    physical_passed = available >= setup.MODEL["size"] + headroom
    ...
    return {
        ...
        "policy": "physical: all resident Q4 weights + 3 GiB; additional commit: 3 GiB for KV/heap/headroom with read-only mmap and no repack",
                ...
                while not stop_monitor.wait(0.5) and process.poll() is None:
                    try:
                        ...
                        if time.monotonic() - load_started >= TOTAL_SECONDS:
                            ...
                            process.kill()
                            return
                        cause = guard.observe(state, time.monotonic())
                        if cause:
                            summary["memoryGuardAborted"] = True
                            summary["memoryGuardAbortReason"] = cause
                            process.kill()
                            return
