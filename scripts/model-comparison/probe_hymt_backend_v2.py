"""V2 7B Q8 backend smoke: non-mmap loading to reduce transient copy memory.
...
Only --load-mode none differs from frozen v1 runtime arguments. This preserves
weights/context/sampling; mmap-copy peak is a hypothesis, not an established cause.
"""
...
def server_command(backend, threads, port, key):
    ...
    argv = [str(directory / "llama-server.exe"), "--model", str(fixed.DEST / fixed.MODEL["name"]),
            ...
            "--device", "none" if backend == "cpu" else "SYCL0", "--load-mode", "none", "--fit", "off",
...
def run(args):
    ...
    summary = {"version": "hymt-backend-smoke-v2", "loadMode": "none", "status": "failed", "backend": args.backend,
