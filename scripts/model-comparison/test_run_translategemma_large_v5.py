...
import run_translategemma_large_v5 as run

IDENTITY = {"pid": 123, "creationTicks": 456}
CPU = {"cpuKernelTicks": 10000000, "cpuUserTicks": 20000000, "cpuKernelSeconds": 1.0,
       "cpuUserSeconds": 2.0, "cpuTotalSeconds": 3.0, "priorityClass": 0x4000, "belowNormalVerified": True}
...
class OrchestrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.limiter.apply_child.return_value = {"flags": 6, "pid": 123, "creationTicks": 456}
    ...
    def test_complete_mock_run_and_exclusive_output(self):
        with self.mocked():
            ...
            self.assertEqual(self.owner.spawn.call_args.kwargs["creationflags"], 0x4000)
            self.owner.assert_owned.assert_called()
            self.assertTrue(summary["childProcessStopped"])
            self.assertEqual(summary["version"], "translategemma-large-screen-v5")
            self.assertIn("memoryAfterStop", summary)
            self.assertEqual(summary["responseFilesSha256"], {"fixture-01-response.json": "fixturehash"})
            self.assertFalse(summary["requestTokenProgressAvailable"])
    ...
    def test_spawn_cleanup_unconfirmed_cannot_inherit_stop_none_success(self):
        for code in ("owned_child_creation_cleanup_failed", "unowned_child_cleanup_failed"):
            with self.subTest(code=code):
                self.args.output = self.output / code
                self.owner.spawn.side_effect = run.process_owner.ProcessOwnershipError(code)
                ...
                self.assertTrue(summary["childCreationCleanupUnconfirmed"])
                self.assertEqual(summary["error"], code)
                self.assertEqual(summary["ownershipErrorCode"], code)
                self.assertEqual(summary["cleanupError"], code)
                self.assertEqual(summary["recordedCount"], 1)
                self.client.request.assert_not_called()
                saved = json.loads((self.args.output / "summary.json").read_text(encoding="utf-8"))
                self.assertFalse(saved["childProcessStopped"])
                self.assertEqual(saved["cleanupError"], code)

    def test_spawn_ownership_failure_keeps_specific_code(self):
        code = "child_ownership_not_verified"
        self.owner.spawn.side_effect = run.process_owner.ProcessOwnershipError(code)
        with self.mocked():
            summary = run.run(self.args)
        self.assertEqual(summary["status"], "failed")
        self.assertEqual(summary["error"], code)
        self.assertEqual(summary["ownershipErrorCode"], code)
        self.assertFalse(summary["childCreationCleanupUnconfirmed"])
        self.client.request.assert_not_called()
    ...
    def test_final_synchronous_peak_failure_cannot_finish_successfully(self):
        ...
        self.monitor.checked_sample.side_effect = [state, state, state, state,
    ...
    def test_initial_priority_refusal_prevents_any_localhost_request(self):
        self.monitor.checked_sample.side_effect = run.RunError("native_priority_not_below_normal")
        with self.mocked():
            result = run.run(self.args)
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["error"], "native_priority_not_below_normal")
        self.assertTrue(result["childProcessStopped"])
        self.client.request.assert_not_called()

    def test_http_exception_preserves_failed_denominator_and_request_state(self):
        original = self.client.request.side_effect
        def request(endpoint, *args, **kwargs):
            if endpoint == "/completion":
                raise ConnectionResetError("SECRET RESPONSE AND KEY")
            return original(endpoint, *args, **kwargs)
        self.client.request.side_effect = request
        with self.mocked():
            result = run.run(self.args)
        self.assertEqual(result["count"], 0)
        self.assertEqual(result["recordedCount"], 1)
        self.assertEqual(result["error"], "ConnectionResetError")
        self.monitor.end_request.assert_called_once_with(False)
        self.assertNotIn("SECRET", json.dumps(result))
        self.assertEqual(json.loads((self.output / "predictions.jsonl").read_text())["status"], "failed")

    def test_late_request_end_preserves_returned_raw_before_guard_failure(self):
        def late_end(received):
            self.assertTrue(received)
            self.monitor.abort_reason = "request_time_limit"
            self.monitor.check.side_effect = run.RunError("request_time_limit")
            self.monitor.receipt.return_value = {"abortReason": "request_time_limit",
                                                 "lastRequestDeadlineExceededAtEnd": True}
        self.monitor.end_request.side_effect = late_end
        with self.mocked():
            result = run.run(self.args)
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["error"], "request_time_limit")
        self.assertTrue(result["childProcessStopped"])
        self.assertTrue(result["memoryOrTimeGuardAborted"])
        self.assertEqual(result["count"], 0)
        raw_path = self.output / "fixture-01-response.json"
        self.assertEqual(json.loads(raw_path.read_text(encoding="utf-8")), response())
        self.assertEqual(result["responseFilesSha256"], {raw_path.name: "fixturehash"})

    def test_late_request_without_response_keeps_failure_and_creates_no_raw(self):
        original = self.client.request.side_effect
        def request(endpoint, *args, **kwargs):
            if endpoint == "/completion":
                raise TimeoutError("SECRET RESPONSE AND KEY")
            return original(endpoint, *args, **kwargs)
        def late_end(received):
        ...
        self.assertTrue(result["childProcessStopped"])
        self.assertEqual(result["recordedCount"], 1)
        self.assertEqual(result["responseFilesSha256"], {})
        self.assertFalse((self.output / "fixture-01-response.json").exists())
        self.assertNotIn("SECRET", json.dumps(result))

    def test_raw_response_mutation_rejects_final_integrity(self):
        calls = 0
        def digest(path):
            nonlocal calls
            if path.name == "fixture-01-response.json":
                calls += 1
                return "initial" if calls == 1 else "changed"
            return "fixturehash"
        with self.mocked(), patch.object(run.setup, "digest", side_effect=digest):
            result = run.run(self.args)
        self.assertEqual(result["status"], "failed")
        self.assertEqual(result["error"], "raw_response_changed")
        self.assertNotIn("integrityVerified", result)
        self.assertTrue(result["childProcessStopped"])


class MemoryTests(unittest.TestCase):
    def test_slow_request_survives_old_deadline_and_stops_at_two_hours(self):
        self.assertEqual((run.STARTUP_SECONDS, run.REQUEST_SECONDS, run.TOTAL_SECONDS),
                         (1800, 7200, 86400))
        with tempfile.TemporaryDirectory() as tmp:
            process = MagicMock()
            process.poll.return_value = None
            monitor = run.MemoryMonitor(process, MagicMock(), Path(tmp) / "samples", 0, 0, owned_identity=IDENTITY, row_ids=["fixture-01"])
            monitor.ready()
            try:
                with patch.object(run.time, "monotonic", return_value=100):
                    monitor.begin_request("fixture-01")
                self.assertEqual(monitor.request_deadline, 7300)
                monitor.observe(self.state(), 1900)
                monitor.observe(self.state(), 7299.9)
                process.kill.assert_not_called()
                monitor.observe(self.state(), 7300)
                self.assertEqual(monitor.abort_reason, "request_time_limit")
    ...
    def state(self, physical=None, commit=None, working=10):
        return {"availablePhysical": run.GIB if physical is None else physical,
                ...
                "child": {"workingSetBytes": working, "peakWorkingSetBytes": working,
                          "privateBytes": 20, "pageFaultCount": 3, **IDENTITY, **CPU}}
    ...
    def test_monitor_limits_and_sample_preservation(self):
        for reason in ("memory_physical_low", "working_set_budget_exceeded", "request_time_limit", "total_time_limit"):
            with self.subTest(reason=reason), tempfile.TemporaryDirectory() as tmp:
                ...
                monitor = run.MemoryMonitor(process, MagicMock(), path, 0, 0, owned_identity=IDENTITY, row_ids=["fixture-01"])
                monitor.ready()
                try:
        ...
        for budget in (8, 9, 10, 11, 12):
            with self.subTest(budget=budget), tempfile.TemporaryDirectory() as tmp:
                ...
                monitor = run.MemoryMonitor(process, MagicMock(), Path(tmp) / "samples", 0, 0,
                                            maximum_working_set=maximum, owned_identity=IDENTITY, row_ids=["fixture-01"])
                try:
    ...
    def test_peak_excess_stops_child_even_after_current_working_set_recovers(self):
        for budget in (8, 9, 10, 11, 12):
            with self.subTest(budget=budget), tempfile.TemporaryDirectory() as tmp:
                ...
                process.poll.return_value = None
                monitor = run.MemoryMonitor(process, MagicMock(), Path(tmp) / "samples", 0, 0,
                                            maximum_working_set=budget * run.GIB, owned_identity=IDENTITY, row_ids=["fixture-01"])
                try:
    ...
    def test_checked_sample_catches_peak_before_any_periodic_tick(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            process.poll.return_value = None
            monitor = run.MemoryMonitor(process, MagicMock(), Path(tmp) / "samples", 0, 0,
                                        maximum_working_set=8 * run.GIB, owned_identity=IDENTITY, row_ids=["fixture-01"])
...
class TelemetryTests(unittest.TestCase):
    def setUp(self):
        ...
        self.process = MagicMock()
        self.process.pid, self.process._handle = 123, 999
        self.process.poll.return_value = None
        self.limiter = MagicMock()
        self.monitor = run.MemoryMonitor(self.process, self.limiter, self.path, 0, 0,
                                         owned_identity=IDENTITY, row_ids=["fixture-01"])
        self.addCleanup(self.monitor.close)

    def state(self):
        return {"availablePhysical": run.GIB, "availablePageFile": run.GIB,
                "child": {**IDENTITY, **CPU, "workingSetBytes": 10, "peakWorkingSetBytes": 10,
                          "privateBytes": 20, "pageFaultCount": 3}}

    def saved(self):
        return [json.loads(line) for line in self.path.read_text(encoding="utf-8").splitlines()]

    def kernel(self, *, pid=123, creation=456, priority=0x4000):
        kernel = MagicMock()
        kernel.GetProcessId.return_value = pid
        kernel.GetPriorityClass.return_value = priority
        def times(handle, *pointers):
            self.assertEqual(handle, 999)
            for pointer, value in zip(pointers, (creation, 0, 10000000, 20000000)):
                entry = ctypes.cast(pointer, ctypes.POINTER(run.wintypes.FILETIME)).contents
                entry.dwLowDateTime, entry.dwHighDateTime = value & 0xffffffff, value >> 32
            return 1
        kernel.GetProcessTimes.side_effect = times
        return kernel

    def test_cpu_and_priority_are_bound_to_retained_handle_creation(self):
        kernel = self.kernel()
        self.assertEqual(run.owned_cpu_sample(self.process, IDENTITY, kernel), CPU)
        kernel.GetProcessId.assert_called_once_with(999)
        kernel.GetPriorityClass.assert_called_once_with(999)
        for changed in ({"pid": 124}, {"creation": 457}):
            with self.subTest(changed=changed), self.assertRaisesRegex(run.RunError, "mismatch"):
                run.owned_cpu_sample(self.process, IDENTITY, self.kernel(**changed))
        kernel = self.kernel()
        kernel.GetProcessTimes.return_value = 0
        kernel.GetProcessTimes.side_effect = None
        with self.assertRaisesRegex(run.RunError, "cpu_query_failed"):
            run.owned_cpu_sample(self.process, IDENTITY, kernel)
        with self.assertRaisesRegex(run.RunError, "priority_query_failed"):
            run.owned_cpu_sample(self.process, IDENTITY, self.kernel(priority=0))

    def test_live_identity_mismatch_cannot_query_cpu(self):
        self.limiter.sample_child.return_value = self.state()["child"] | {"creationTicks": 457}
        with patch.object(run, "owned_cpu_sample") as cpu, self.assertRaisesRegex(run.RunError, "identity_changed"):
            self.monitor.sample()
        cpu.assert_not_called()

    def test_non_below_normal_is_recorded_and_owned_child_stopped(self):
        state = self.state()
        state["child"].update(run.owned_cpu_sample(self.process, IDENTITY, self.kernel(priority=0x20)))
        self.monitor.observe(state, 1)
        self.assertEqual(self.monitor.abort_reason, "native_priority_not_below_normal")
        self.assertEqual(self.saved()[0]["child"]["priorityClass"], 0x20)
        self.assertFalse(self.saved()[0]["child"]["belowNormalVerified"])
        self.process.kill.assert_called_once()

    def test_phase_transition_and_request_failure_retained_without_prose(self):
        self.monitor.observe(self.state(), 0)
        self.monitor.ready()
        self.monitor.observe(self.state(), 0.1)
        with patch.object(run.time, "monotonic", return_value=0.2):
            self.monitor.begin_request("fixture-01")
        self.monitor.observe(self.state(), 0.3)
        with patch.object(run.time, "monotonic", return_value=1.2):
            self.monitor.end_request(False)
        self.monitor.observe(self.state(), 1.3)
        rows = self.saved()
        self.assertEqual([row["phase"] for row in rows],
                         ["startup", "runtime-validation", "generation-request", "request-failed"])
        self.assertEqual(rows[-1]["requestElapsedSeconds"], 1.0)
        self.assertFalse(rows[-1]["requestActive"])
        self.assertIsNone(rows[-1]["requestTokenProgress"])
        self.assertEqual(self.monitor.receipt()["lastPhase"], "request-failed")
        for phase, row in (("SECRET PROSE", None), ("generation-request", "SECRET KEY")):
            with self.assertRaises(run.RunError):
                self.monitor.set_phase(phase, row)

    def test_request_end_latches_exact_deadline_without_kill_or_nested_lock(self):
        for received in (True, False):
            for elapsed in (run.REQUEST_SECONDS - 0.001, run.REQUEST_SECONDS, run.REQUEST_SECONDS + 1):
                with self.subTest(received=received, elapsed=elapsed):
                    child = MagicMock()
                    child.poll.return_value = None
                    monitor = run.MemoryMonitor(child, self.limiter,
                        self.path.with_name(f"{received}-{elapsed}.jsonl"), 0, 0,
                        owned_identity=IDENTITY, row_ids=["fixture-01"])
                    self.addCleanup(monitor.close)
                    monitor.ready()
                    with patch.object(run.time, "monotonic", return_value=0):
                        monitor.begin_request("fixture-01")
                    with patch.object(run.time, "monotonic", return_value=elapsed):
                        monitor.end_request(received)
                    exceeded = elapsed >= run.REQUEST_SECONDS
                    self.assertEqual(monitor.abort_reason, "request_time_limit" if exceeded else None)
                    self.assertEqual(monitor.receipt()["lastRequestDeadlineExceededAtEnd"], exceeded)
                    self.assertIsNone(monitor.request_deadline)
                    child.kill.assert_not_called()
                    # A delayed observation must not erase the latched timeout.
                    monitor.observe(self.state(), elapsed)
                    self.assertEqual(monitor.abort_reason, "request_time_limit" if exceeded else None)
                    if exceeded:
                        with self.assertRaisesRegex(run.RunError, "request_time_limit"):
                            monitor.check()

    def test_heartbeat_gap_uses_all_observations_and_window_max_survives(self):
        for now in (0, 0.5, 2.5, 3, 3.5, 4, 4.5, 5):
            self.monitor.observe(self.state(), now)
        rows = self.saved()
        self.assertEqual(len(rows), 2)
        self.assertEqual(rows[-1]["heartbeatGapSeconds"], 0.5)
        self.assertEqual(rows[-1]["maximumHeartbeatGapSincePreviousRecordSeconds"], 2.0)
        self.assertEqual(rows[-1]["recordGapSeconds"], 5.0)
        self.monitor.ready()
        self.monitor.observe(self.state(), 1689.281)
        self.assertEqual(self.saved()[-1]["heartbeatGapSeconds"], 1684.281)
        self.assertEqual(self.monitor.receipt()["maximumHeartbeatGapSeconds"], 1684.281)
        self.assertEqual(self.monitor.receipt()["firstCpuTicks"], self.monitor.receipt()["lastCpuTicks"])
        self.process.kill.assert_not_called()

    def test_exit_is_from_original_handle_and_never_fakes_memory_sample(self):
        ...
        with patch.object(run, "owned_cpu_sample") as cpu:
            self.monitor._run()
            with self.assertRaisesRegex(run.RunError, "runtime_exited"):
                self.monitor.check()
        self.limiter.sample_child.assert_not_called()
        cpu.assert_not_called()
        self.assertEqual(len(self.saved()), 1)
        receipt = self.monitor.receipt()["ownedChildExitObserved"]
        self.assertEqual((receipt["pid"], receipt["creationTicks"], receipt["exitCode"]), (123, 456, 7))
        self.process.kill.assert_not_called()

    def test_telemetry_only_allows_numeric_resource_fields_and_fixed_metadata(self):
        state = self.state() | {"source": "PRIVATE SOURCE", "command": "SECRET KEY"}
        state["child"]["translation"] = "PRIVATE TRANSLATION"
        self.monitor.observe(state, 1)
        text = self.path.read_text(encoding="utf-8") + json.dumps(self.monitor.receipt())
        for forbidden in ("PRIVATE", "SECRET", '"translation"', '"command"', '"source"'):
            self.assertNotIn(forbidden, text)
        self.monitor.set_phase("prompt-preflight")
        with self.assertRaisesRegex(run.RunError, "system_memory"):
            self.monitor.observe(self.state() | {"load": "SECRET KEY"}, 2)


class UnchangedV4ContractTests(unittest.TestCase):
    def test_generation_and_resource_contract_functions_are_literal_v4_ast(self):
        original = Path(run.__file__).with_name("run_translategemma_large_v4.py")
        self.assertEqual(hashlib.sha256(original.read_bytes()).hexdigest(), run.BASE_V4_SHA256)
        old = ast.parse(original.read_text(encoding="utf-8"))
        new = ast.parse(Path(run.__file__).read_text(encoding="utf-8"))
        nodes = lambda tree: {node.name: ast.dump(node, include_attributes=False) for node in tree.body
                              if isinstance(node, (ast.FunctionDef, ast.ClassDef))}
        old_nodes, new_nodes = nodes(old), nodes(new)
        for name in ("render_prompt", "validate_prompt_tokens", "eog_from_log", "validate_runtime_tokens",
                     "validate_active_bias", "memory_requirements", "check_memory", "validate_memory_shape",
                     "MemoryGuard", "prediction", "server_command", "file_stamps", "code_hashes"):
            with self.subTest(name=name):
                self.assertEqual(new_nodes[name], old_nodes[name])
        assignments = lambda tree: {node.targets[0].id: ast.dump(node.value, include_attributes=False)
                                    for node in tree.body if isinstance(node, ast.Assign) and isinstance(node.targets[0], ast.Name)}
        old_values, new_values = assignments(old), assignments(new)
        for name in ("CONTEXT_SIZE", "MAX_NEW_TOKENS", "SETTINGS", "STARTUP_SECONDS", "REQUEST_SECONDS", "TOTAL_SECONDS",
                     "MONITOR_INTERVAL", "SAMPLE_RECORD_INTERVAL", "MINIMUM_FREE_MEMORY", "MEMORY_LOW_SECONDS",
                     "MEMORY_SHAPES", "ORIGINAL_EOG_IDS", "RUNTIME_EOG", "MAXIMUM_WORKING_SET", "MINIMUM_WORKING_SET"):
            with self.subTest(name=name):
                self.assertEqual(new_values[name], old_values[name])
