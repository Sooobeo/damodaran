"""TranslateGemma 27B v5: owned-process telemetry; v4 generation/resource guards retained."""
...
SYSTEM_MEMORY_FIELDS = ("load", "totalPhysical", "availablePhysical", "totalPageFile", "availablePageFile")
CHILD_FIELDS = ("pid", "creationTicks", "workingSetBytes", "peakWorkingSetBytes", "privateBytes", "pageFaultCount",
                "cpuKernelTicks", "cpuUserTicks", "cpuKernelSeconds", "cpuUserSeconds", "cpuTotalSeconds",
                "priorityClass", "belowNormalVerified")
...
@lru_cache(maxsize=1)
def cpu_kernel():
    """Bind Win32 signatures once; no per-observation DLL setup or process open."""
    require(os.name == "nt", "telemetry_requires_windows")
    kernel = ctypes.WinDLL("kernel32", use_last_error=True)
    kernel.GetProcessId.argtypes, kernel.GetProcessId.restype = [wintypes.HANDLE], wintypes.DWORD
    kernel.GetPriorityClass.argtypes, kernel.GetPriorityClass.restype = [wintypes.HANDLE], wintypes.DWORD
    kernel.GetProcessTimes.argtypes = [wintypes.HANDLE] + [ctypes.POINTER(wintypes.FILETIME)] * 4
    kernel.GetProcessTimes.restype = wintypes.BOOL
    return kernel


def owned_cpu_sample(process, identity, kernel=None):
    """Read only the retained owned handle, after limiter.sample_child's Job/identity check.

    Never reopen by PID. The second creation-time check binds CPU and priority
    to the same process as the independently verified memory observation.
    """
    require(process.poll() is None, "runtime_exited")
    require(type(process.pid) is int and process.pid == identity["pid"], "telemetry_owned_pid_mismatch")
    handle = int(process._handle)
    require(handle > 0, "telemetry_owned_handle_invalid")
    if kernel is None:
        kernel = cpu_kernel()
    require(kernel.GetProcessId(handle) == identity["pid"], "telemetry_owned_pid_mismatch")
    times = [wintypes.FILETIME() for _ in range(4)]
    require(bool(kernel.GetProcessTimes(handle, *(ctypes.byref(value) for value in times))), "telemetry_cpu_query_failed")
    ticks = [value.dwHighDateTime << 32 | value.dwLowDateTime for value in times]
    require(ticks[0] == identity["creationTicks"], "telemetry_owned_creation_mismatch")
    priority = kernel.GetPriorityClass(handle)
    require(type(priority) is int and priority > 0, "telemetry_priority_query_failed")
    require(process.poll() is None, "runtime_exited")
    return {"cpuKernelTicks": ticks[2], "cpuUserTicks": ticks[3],
            "cpuKernelSeconds": ticks[2] / 10000000, "cpuUserSeconds": ticks[3] / 10000000,
            "cpuTotalSeconds": (ticks[2] + ticks[3]) / 10000000,
            "priorityClass": priority, "belowNormalVerified": priority == BELOW_NORMAL_PRIORITY_CLASS}
...
class MemoryMonitor:
    ...
    def __init__(self, process, limiter, path, run_started, startup_started,
                 maximum_working_set=MAXIMUM_WORKING_SET, *, owned_identity, row_ids):
        ...
        self.request_deadline_exceeded_at_end = False
        require(isinstance(owned_identity, dict) and all(type(owned_identity.get(k)) is int and owned_identity[k] > 0
                for k in ("pid", "creationTicks")), "telemetry_owned_identity_required")
        self.owned_identity = {k: owned_identity[k] for k in ("pid", "creationTicks")}
        require(isinstance(row_ids, list) and bool(row_ids) and len(set(row_ids)) == len(row_ids)
                and all(isinstance(row_id, str) and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,79}", row_id)
                        for row_id in row_ids), "telemetry_row_ids_invalid")
        self.row_ids = frozenset(row_ids)
        self.phase, self.row_id, self.phase_dirty = "startup", None, True
        self.request_started = self.request_started_utc = self.request_finished = None
        self.last_observed = self.last_observed_utc = None
        self.maximum_heartbeat_gap = self.pending_maximum_gap = 0.0
        self.child_exit_observed = None
        self.first_cpu_ticks = self.last_cpu_ticks = None
    ...
    def sample(self):
        child = self.limiter.sample_child(self.process)
        require(all(child.get(k) == v for k, v in self.owned_identity.items()), "telemetry_owned_identity_changed")
        cpu = owned_cpu_sample(self.process, self.owned_identity)
        return {**screen.memory_status(), "child": {**child, **cpu}}
    ...
    def _record(self, state, now, force=False):
        # Whitelist output, even if an upstream diagnostic later adds free text.
        require(all(type(state[k]) is int and state[k] >= 0 for k in SYSTEM_MEMORY_FIELDS if k in state),
                "invalid_system_memory_telemetry")
        state = {**{k: state[k] for k in SYSTEM_MEMORY_FIELDS if k in state},
                 "child": {k: state["child"][k] for k in CHILD_FIELDS}}
        observed_utc = utc_now()
        gap = None if self.last_observed is None else max(0.0, now - self.last_observed)
        previous_utc = self.last_observed_utc
        self.last_observed, self.last_observed_utc = now, observed_utc
        if gap is not None:
            self.maximum_heartbeat_gap = max(self.maximum_heartbeat_gap, gap)
            self.pending_maximum_gap = max(self.pending_maximum_gap, gap)
        with self.lock:
            phase, row_id, dirty = self.phase, self.row_id, self.phase_dirty
            request_start, request_utc, request_end = self.request_started, self.request_started_utc, self.request_finished
            # Clearing under the same lock leaves subsequent transitions pending.
            self.phase_dirty = False
        self.observations += 1
        child = state["child"]
        cpu_ticks = child["cpuKernelTicks"] + child["cpuUserTicks"]
        if self.first_cpu_ticks is None:
            self.first_cpu_ticks = cpu_ticks
        ...
        if force or dirty or self.last_recorded is None or now - self.last_recorded >= SAMPLE_RECORD_INTERVAL:
            telemetry = {"telemetryVersion": "owned-phase-resource-v1", "observedAtUTC": observed_utc,
                         "previousObservationUTC": previous_utc, "phase": phase, "rowId": row_id,
                         "requestStartUTC": request_utc,
                         "requestElapsedSeconds": None if request_start is None else round(max(0.0, (now if request_end is None else request_end) - request_start), 6),
                         "requestActive": request_start is not None and request_end is None,
                         "heartbeatGapSeconds": None if gap is None else round(gap, 6),
                         "maximumHeartbeatGapSincePreviousRecordSeconds": round(self.pending_maximum_gap, 6),
                         "recordGapSeconds": None if self.last_recorded is None else round(now - self.last_recorded, 6),
                         "requestTokenProgress": None, "requestTokenProgressAvailable": False}
            self.stream.write(json.dumps({"seconds": round(now - self.run_started, 3), **telemetry, **state}) + "\n")
            self.stream.flush()
            self.records += 1
            self.last_recorded = now
            self.pending_maximum_gap = 0.0
    ...
    def observe(self, state, now):
        ...
        require(all(type(child.get(k)) is int and child[k] == v for k, v in self.owned_identity.items()), "telemetry_owned_identity_changed")
        require(all(type(child.get(k)) is int and child[k] >= 0 for k in ("cpuKernelTicks", "cpuUserTicks"))
                and all(type(child.get(k)) in (int, float) and child[k] >= 0 for k in
                        ("cpuKernelSeconds", "cpuUserSeconds", "cpuTotalSeconds"))
                and type(child.get("priorityClass")) is int and child["priorityClass"] > 0
                and type(child.get("belowNormalVerified")) is bool
                and child["belowNormalVerified"] == (child["priorityClass"] == BELOW_NORMAL_PRIORITY_CLASS),
                "invalid_child_cpu_priority_sample")
        with self.lock:
        ...
        reason = reason or (None if child["belowNormalVerified"] else "native_priority_not_below_normal")
    ...
    def _run(self):
        while not self.stop_event.wait(MONITOR_INTERVAL):
            ...
            try:
                ...
                self.note_child_exit()
                self._abort("runtime_exited" if self.child_exit_observed is not None else "memory_monitor_failed")
                return
    ...
    def ready(self):
        ...
        self.set_phase("runtime-validation")

    def set_phase(self, phase, row_id=None):
        require(phase in PHASES and (row_id is None or row_id in self.row_ids), "telemetry_phase_or_row_invalid")
        with self.lock:
            self.phase_dirty = self.phase_dirty or self.phase != phase or self.row_id != row_id
            self.phase, self.row_id = phase, row_id

    def begin_request(self, row_id):
        self.check()
        require(row_id in self.row_ids, "telemetry_row_id_unknown")
        with self.lock:
            self.request_started = time.monotonic()
            self.request_started_utc, self.request_finished = utc_now(), None
            self.request_deadline = self.request_started + REQUEST_SECONDS
            self.request_deadline_exceeded_at_end = False
            self.phase, self.row_id, self.phase_dirty = "generation-request", row_id, True

    def end_request(self, response_received):
        require(type(response_received) is bool, "telemetry_response_state_invalid")
        with self.lock:
            self.request_finished = time.monotonic()
            self.request_deadline_exceeded_at_end = (self.request_deadline is not None
                                                    and self.request_finished >= self.request_deadline)
            if self.request_deadline_exceeded_at_end:
                # Latch under the same lock as the deadline. Do not raise or
                # kill here: a returned object must be saved before check(),
                # then normal owned cleanup handles either HTTP outcome.
                self.abort_reason = self.abort_reason or "request_time_limit"
            self.request_deadline = None
            self.phase = "response-validation" if response_received else "request-failed"
            self.phase_dirty = True

    def note_child_exit(self):
        code = self.process.poll()
        if type(code) is int and self.child_exit_observed is None:
            self.child_exit_observed = {**self.owned_identity, "exitCode": code, "observedAtUTC": utc_now(),
                                        "seconds": round(time.monotonic() - self.run_started, 6),
                                        "basis": "retained owned Popen handle; no PID reopen; no fresh memory sample"}
    ...
    def receipt(self):
        return {"abortReason": self.abort_reason, "monitorError": self.monitor_error,
                "telemetryVersion": "owned-phase-resource-v1", "ownedIdentity": self.owned_identity,
                "lastPhase": self.phase, "lastRowId": self.row_id,
                "lastRequestStartUTC": self.request_started_utc,
                "lastRequestDeadlineExceededAtEnd": self.request_deadline_exceeded_at_end,
                "lastRequestElapsedSeconds": None if self.request_started is None else round(
                    (self.request_finished if self.request_finished is not None else time.monotonic()) - self.request_started, 6),
                "lastObservationUTC": self.last_observed_utc,
                "maximumHeartbeatGapSeconds": round(self.maximum_heartbeat_gap, 6),
                "ownedChildExitObserved": self.child_exit_observed,
                "firstCpuTicks": self.first_cpu_ticks, "lastCpuTicks": self.last_cpu_ticks,
                "cpuTickUnitSeconds": 0.0000001, "requiredPriorityClass": BELOW_NORMAL_PRIORITY_CLASS,
...
def run(args):
    ...
    summary = {"version": VERSION, "status": "running", "startedAt": datetime.now(timezone.utc).isoformat(),
               "telemetryVersion": "owned-phase-resource-v1", "copiedFromV4Sha256": BASE_V4_SHA256,
               "requiredNativePriorityClass": BELOW_NORMAL_PRIORITY_CLASS,
               "nativePrioritySetAfterSpawnByRunner": False, "requestTokenProgressAvailable": False,
               "rawNativeLogsRetained": False, "childCreationCleanupUnconfirmed": False,
               "responseFilesSha256": {},
               "outerWatchdogLimitation": "An external 86400-second watchdog starting before this runner may terminate it before native finalization and summary; no time budget extended",
    ...
    try:
        ...
        for row in rows:
            require(isinstance(row["id"], str) and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,79}", row["id"]),
                    "telemetry_row_ids_invalid")
        ...
        monitor = MemoryMonitor(process, limiter, output / "memory-samples.jsonl", run_started, startup,
                                ...
                                owned_identity=summary["childWorkingSetLimit"], row_ids=[row["id"] for row in rows])
        diagnostics = StartupDiagnostics(process.stdout)
        # Read actual BelowNormal and identity before the first localhost request.
        ...
        monitor.set_phase("prompt-preflight")
        tokenized = []
        ...
        with (output / "predictions.jsonl").open("x", encoding="utf-8", newline="\n") as stream:
            for row, prompt, tokens in zip(rows, prompts, tokenized):
                ...
                try:
                    ...
                    monitor.end_request(response_received)
                elapsed = time.monotonic() - started
                # Preserve the returned object even if validation rejects its protocol.
                raw_path = output / (row["id"] + "-response.json")
                screen.write_once(raw_path, response)
                summary["responseFilesSha256"][raw_path.name] = setup.digest(raw_path)
                monitor.check()
                item = prediction(row, prompt, tokens, response, elapsed, vocab_size)
                item["rawResponseFile"] = raw_path.name
                item["rawResponseSha256"] = summary["responseFilesSha256"][raw_path.name]
        ...
        monitor.set_phase("final-integrity", failed_id)
        unchanged()
        require(all(setup.digest(output / name) == digest for name, digest in summary["responseFilesSha256"].items()),
                "raw_response_changed")
        monitor.checked_sample()
        summary["integrityVerified"] = True
        summary["status"] = "completed"
    except BaseException as exc:
        if isinstance(exc, process_owner.ProcessOwnershipError):
            # spawn may raise before returning a Popen. None then means no
            # returned handle, not proof that the native child was stopped.
            summary["ownershipErrorCode"] = exc.code
            summary["childCreationCleanupUnconfirmed"] = exc.code in (
                "owned_child_creation_cleanup_failed", "unowned_child_cleanup_failed")
        summary.update(status="failed", phase=phase, failedRowId=failed_id,
                       error=exc.code if isinstance(exc, process_owner.ProcessOwnershipError)
                       else str(exc) if isinstance(exc, RunError) else type(exc).__name__)
    finally:
        if diagnostics:
            diagnostics.discard()
        if monitor:
        ...
        try:
            stopped = transport.stop_process(process)
            summary["childProcessStopped"] = stopped and not summary["childCreationCleanupUnconfirmed"]
            if not summary["childProcessStopped"]:
                summary.update(status="failed", cleanupError=summary["ownershipErrorCode"]
                               if summary["childCreationCleanupUnconfirmed"] else "owned_process_cleanup_failed")
        except BaseException:
            summary.update(status="failed", childProcessStopped=False, cleanupError=summary["ownershipErrorCode"]
                           if summary["childCreationCleanupUnconfirmed"] else "owned_process_cleanup_failed")
        finally:
            if monitor:
                monitor.note_child_exit()
                summary["memoryMonitoring"] = monitor.receipt()
