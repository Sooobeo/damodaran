...
VERSION = "linguistic-development-assistant-review-summary-v4"
...
def review_code_inventory():
    paths = (Path(__file__).resolve(), Path(__file__).with_name("prepare_linguistic_reviews_v4.py").resolve(),
             Path(v3_review_evidence.__file__).resolve(), Path(v4_review_evidence.__file__).resolve(),
             Path(__file__).with_name("linguistic_screen.py").resolve())
...
def summarize(prepared_directory, review_paths, output_path, root=ROOT):
    ...
    require(manifest.get("reviewEvidenceVersion") == "v4", "prepared_review_evidence_version_differs")
    ...
    for system in key["systems"]:
        ...
        v4_review_evidence.check_artifacts(directory, summary, predictions, evidence)
    ...
    require(manifest.get("v4ArtifactFilesSha256") == v4_review_evidence.artifact_inventory_sha(evidence),
