...
PREPARED_VERSION = "real-reading-blind-review-packets-v4"
SUMMARY_VERSION = "real-reading-assistant-review-summary-v4"
...
def code_inventory():
    paths = [Path(__file__).resolve(), Path(reader.__file__).resolve(), Path(v3_review_evidence.__file__).resolve(), Path(v4_review_evidence.__file__).resolve()]
    paths += [Path(__file__).with_name(name) for name in
              ("prepare_reading_reviews_v4.py", "summarize_reading_reviews_v4.py")]
...
def producer(directory, clean, identity, evidence, expected=_UNSET):
    ...
    if version in ("translategemma-large-screen-v2", "translategemma-large-screen-v3", "translategemma-large-screen-v4"):
