"""Preserve one observed tail-recovery/reading row without claiming full dev18 completion."""
...
VERSION = 'tg27-recovery-root-review-v1'
TAIL_RUN = 'translategemma-27b-v5-tail-retry-20260912-b8'
READING_RUN = 'translategemma-27b-v5-tail-recovery-20260912-b8'
...
OUTPUT = '.training/quality-evaluation/tg27-recovery-root-review-20260912/partial'
...
def recovery_run_name(cohort, sid):
    require((cohort == 'dev18' and sid in ('LDEV26-017', 'LDEV26-018')) or
            (cohort == 'reading6' and sid in [f'REAL26-{n:03d}' for n in range(1, 7)]),
            'only_fixed_recovery_rows_allowed')
    return TAIL_RUN if cohort == 'dev18' else READING_RUN
...
def preserve(review_path, root=ROOT):
    ...
    run = safe(Path('.training/comparisons') / folder / recovery_run_name(cohort, review['id']), root)
