...
PREPARED_VERSION = "real-reading-blind-review-packets-v1"
SUMMARY_VERSION = "real-reading-assistant-review-summary-v1"
...
class Evidence:
    ...
    def unchanged(self):
        for path, expected in list(self.files.items()):
            self.raw(path, expected)


def code_inventory():
    paths = [Path(__file__).resolve(), Path(reader.__file__).resolve(), Path(v3_review_evidence.__file__).resolve()]
    paths += [Path(__file__).with_name(name) for name in
              ("prepare_reading_reviews.py", "summarize_reading_reviews.py")]
...
def producer(directory, clean, identity, evidence, expected=_UNSET):
    ...
    if version in ("translategemma-large-screen-v2", "translategemma-large-screen-v3"):
        ...
        require(version in (None, "hymt30-development-screen-v2", "hymt30-development-screen-v3"), "unsupported_producer_version")
