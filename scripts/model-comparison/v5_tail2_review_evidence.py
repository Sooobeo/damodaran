"""Strict evidence for ONLY the LDEV26-017/018 TG27-v5 recovery run.

This separate consumer supports the existing producer --ids option. It never
certifies a full dev18 run or rewrites the original v5 consumer/failed run.
...
EVIDENCE_CONTRACT = "tg27-v5-fixed-tail2-evidence-v1"
TAIL_IDS = ["LDEV26-017", "LDEV26-018"]
...
def check_v5_evidence(summary, predictions_sha):
    ...
    require(type(summary.get("expectedCount")) is int and summary["expectedCount"] == 2
            and all(type(summary.get(k)) is int and summary[k] == summary["expectedCount"] for k in ("count", "recordedCount")),
            "fixed_tail_two_completed_rows_required")
    ids = summary.get("input", {}).get("selectedIds")
    require(ids == TAIL_IDS, "only_fixed_unfinished_tail_ids_allowed")
...
def check_artifacts(directory, summary, predictions, evidence):
    require(summary.get("version") == VERSION, "tail_retry_requires_original_v5")
