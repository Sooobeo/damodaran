...
VERSION = "linguistic-development-assistant-review-summary-v5"
...
def normalize_producer_summary(summary, predictions_sha):
    ...
    if summary.get("version") == v5_review_evidence.VERSION:
        v5_review_evidence.check_v5_evidence(summary, predictions_sha)
        projected = normalize_producer_summary(v5_review_evidence.v4_projection(summary), predictions_sha)
        return {**projected, "schema": summary["version"]}
    if summary.get("version") in v4_review_evidence.VERSIONS:
        v4_review_evidence.check_v4_evidence(summary, predictions_sha)
    if summary.get("version") in v3_review_evidence.VERSIONS:
        v3_review_evidence.check_v3_evidence(summary, predictions_sha)
    if summary.get("version") in ("translategemma-large-screen-v1", "translategemma-large-screen-v2", "translategemma-large-screen-v3", "translategemma-large-screen-v4"):
    ...
    require(summary.get("version") in (None, "hymt30-development-screen-v1", "hymt30-development-screen-v2", "hymt30-development-screen-v3", "hymt30-development-screen-v4"),
...
def review_code_inventory():
    paths = (Path(__file__).resolve(), Path(__file__).with_name("prepare_linguistic_reviews_v5.py").resolve(),
             Path(v3_review_evidence.__file__).resolve(), Path(v4_review_evidence.__file__).resolve(),
             Path(__file__).with_name("linguistic_screen.py").resolve(), Path(v5_review_evidence.__file__).resolve())
...
def summarize(prepared_directory, review_paths, output_path, root=ROOT):
    ...
    require(manifest.get("reviewEvidenceVersion") == "v5", "prepared_review_evidence_version_differs")
    require(manifest.get("reviewCodeFiles") == review_codes, "prepared_review_code_changed")
    ...
    require(manifest.get("v5ArtifactFilesSha256") == v5_review_evidence.artifact_inventory_sha(evidence),
            "prepared_v5_artifact_inventory_changed")
    mapping = key["mapping"]
