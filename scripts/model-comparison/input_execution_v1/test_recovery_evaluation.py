...
class RecoveryEvaluationTests(unittest.TestCase):
    ...
    def test_real_cohort_builder_validator_interface_with_synthetic_run_boundaries(self):
        ...
        from input_execution_v1 import recovery_cohort as cohort_module
        results = []
        for run, rows in ((self.prior, self.rows[:38]), (self.recovered, self.rows[38:])):
            ...
            evidence = {name: cohort_module.ref(self.root, run / (name + ".json" if name != "predictions" else "predictions.jsonl"))
                        for name in ("summary", "plan", "predictions")}
            evidence["files"] = [cohort_module.ref(self.root, path) for path in paths]
            results.append({"rows": rows, "evidence": evidence})
        destination = self.root / cohort_module.COHORT_BASE / "synthetic-interface-cohort"
        with patch.object(cohort_module, "validate_failed_run", return_value=results[0]), \
