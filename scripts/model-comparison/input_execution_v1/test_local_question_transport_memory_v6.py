...
from input_execution_v1 import local_question_transport_memory_v6 as t
...
from input_execution_v1.test_local_question_transport_partial_recovery_v5 import synthetic_partial as synthetic_v5
...
def synthetic_partial(battery, root=None, new_run=None):
    ...
    base = t.tr.BASE + "/synthetic-memory-v6"
    previous_identity, _ = synthetic_v5(battery, root)
    previous = previous_identity["priorRun"]["path"]
    new_run = new_run or base + "/new-v6"
    ids = list(t.contract.IDS)
    partial = {"version": runner.PARTIAL_VERSION,
        "batteryExecution": deepcopy(battery),
        **{key: deepcopy(previous_identity[key]) for key in ("priorRun", "priorClaim", "priorSupersession", "priorReview")},
        ...
        "files": [artifact(base + "/code-" + str(i) + ".json") for i in range(11)],
        "previousPartialRecovery": deepcopy(previous_identity),
        "memoryPolicy": deepcopy(runner.MEMORY_POLICY),
        "originalProfile": deepcopy(runner.v4.PROFILE), "profile": deepcopy(runner.PROFILE),
        "userAuthorization": deepcopy(runner.USER_AUTHORIZATION)}
    source_runs = []
    for path, version, status, expected, completed in ((previous, t.PRIOR_CONTEXT_VERSION, "failed", 64, 9),
...
class PartialTransportTests(unittest.TestCase):
    def setUp(self):
        ...
        for index, context in enumerate(self.native["contexts"]):
            ...
            context["contextProducerVersion"] = t.PRIOR_CONTEXT_VERSION if index < 9 else t.CONTEXT_VERSION
    ...
    def test_logical_roundtrip_preserves_failed9_v4_and_completed55_v6_contexts(self):
        ...
        self.assertEqual([row["contextProducerVersion"] for row in provenance["contexts"]],
                         [t.PRIOR_CONTEXT_VERSION] * 9 + [t.CONTEXT_VERSION] * 55)
    ...
    def test_prior_status_counts_or_context_origin_cannot_change(self):
        ...
        for kind in ("prior-status", "new-count", "context-version", "retained-version", "fresh-version", "context-origin"):
            ...
            elif kind == "retained-version": self.native["contexts"][0]["contextProducerVersion"] = t.CONTEXT_VERSION
            elif kind == "fresh-version": self.native["contexts"][9]["contextProducerVersion"] = t.PRIOR_CONTEXT_VERSION
        ...
        with tempfile.TemporaryDirectory(prefix="partial-loader-boundary-") as temporary:
            ...
            with support.actual_battery_fixture(root) as battery:
                ...
                with patch.object(runner, "partial_freeze_inputs", return_value=deepcopy(expected)):
                    ...
                    self.assertEqual(len(actual["files"]), 11)
                    self.assertEqual(actual["previousPartialRecovery"], partial["previousPartialRecovery"])
                    self.assertEqual(actual["profile"]["minimumStartPhysicalBytes"], 7 * 1024 ** 3)
                    self.assertEqual(actual["originalProfile"]["minimumStartPhysicalBytes"], 9 * 1024 ** 3)
