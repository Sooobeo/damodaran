"""Explicit partial-recovery-v5 logical collection over byte-unchanged v1 exports.
...
VERSION = "input-execution-v1-local-question-transport-partial-recovery-v5"
RUNNER_VERSION = "input-execution-v1-local-question-runner-partial-recovery-v5"
CONTEXT_VERSION = "input-execution-v1-local-question-runner-battery-v4"
SUMMARY_VERSION = "input-execution-v1-local-question-partial-recovery-cohort-v5"
...
def validate_question_provenance(value, partial):
    ...
    for index, (path, version, status, expected, completed) in enumerate((
            (partial["priorRun"]["path"], CONTEXT_VERSION, "failed", 64, 9),
    ...
    for index, row in enumerate(contexts):
        ...
        ev.require(set(row) == {"reviewId", "contextId", "contextProducerVersion", "sourceRun"} and
                   ...
                   row["contextProducerVersion"] == CONTEXT_VERSION and
...
def contents(formal, export, run, root):
    ...
    for context in contexts:
        ev.require(context.get("contextProducerVersion") == CONTEXT_VERSION, "battery_context_producer_mismatch")
