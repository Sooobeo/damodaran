...
from input_execution_v1 import local_question_evaluation_inventory_v2 as local
from input_execution_v1 import recovery_relations_v2 as relations
from input_execution_v1 import local_question_append_evidence_inventory_v2 as app
...
def complete_fixture():
    ...
    manifest = {"version": reval.COHORT_VERSION, "status": "completed_logical_cohort", "validationVersion": reval.lineage.VALIDATION_VERSION,
                "validationLineage": {"syntheticOnly": True}, "sourceRuns": [
    ...
    cohort = {"rows": outputs, "units": units, "annotations": annotations, "prepared": prepared, "manifest": manifest,
              "manifestRef": manifest_ref, "provenance": provenance, "evidence": refs, "code": [], "evaluationLineage": {"syntheticOnly": True}}
    ...
    formal = {"questionProtocol": {"path": "fixture/local-protocol.md", "sha256": "e" * 64}, "bundle": bundle, "recovery": bundle["recovery"], "manifestSha256": "f" * 64, "evidence": refs}
    formal["inventoryCorrection"] = {"version": "synthetic-inventory-correction", "baseExecutionFreeze": {"path": "fixture/base.json", "sha256": "6" * 64}, "correctionFreeze": {"path": "fixture/correction.json", "sha256": "7" * 64}, "files": []}
    ...
    report["recoveryEvidence"] = local.report_recovery_evidence(formal, [*refs, source_ref, grade_ref, *answer_refs], answers)
    ...
    relation_report.update(evaluationLineage=bundle["recovery"]["evaluationLineage"], cohortManifest=manifest, cohortEvidence=cohort_validation["evidence"],
    ...
    result = {"bundle": bundle, "report": report, "outputs": outputs, "relations": relation_report,
              ...
              "recovery": formal["recovery"], "inventoryCorrection": deepcopy(formal["inventoryCorrection"]), "graph": Mock(files={"fixture/report.json": "a" * 64})}
    return result, formal, source, answers, grades, source_ref, grade_ref, answer_refs, cohort_validation


class LocalQuestionAppendEvidenceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_local_answer_model_and_context_count_are_revalidated(self):
        for field, altered in (("models", ["wrong synthetic model"]), ("validatedContexts", 63),
                               ("allAnswersValidatedAndFrozen", False)):
            data, formal, source, answers, grades, sr, gr, ar, _ = deepcopy(self.fixture)
            data["report"]["recoveryEvidence"]["questionAnswering"][field] = altered
            with self.subTest(field=field), self.assertRaisesRegex(ValueError, "not_reproducible"):
    ...
    def test_escaped_destination_is_rejected_before_validation_or_ledger_write(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp).resolve()
            destination = root / original.BASE / "../../../escaped-events"
            with patch.object(app, "validate_final_evidence") as validate, \
                 patch.object(original, "append_prevalidated") as writer:
                for append in (False, True):
                    with self.subTest(append=append), self.assertRaisesRegex(ValueError, "private_development_path_required"):
                        app.prepare(folder="fixture/s5", cohort="fixture/cohort", source_reviews="fixture/reviews.json",
                            grades="fixture/grades.json", report_path="fixture/report.json", relation_folder="fixture/relations",
                            destination=destination, append=append, root=root)
                validate.assert_not_called(); writer.assert_not_called()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(original, "verify_evaluation_freeze"), \
                ...
                with self.assertRaisesRegex(ValueError, "cohort_incomplete"):
                    app.prepare(folder=root / original.BASE / "fixture-s5", cohort="fixture/cohort", source_reviews="fixture/reviews.json",
                        grades="fixture/grades.json", report_path="fixture/report.json", relation_folder="fixture/relations",
    ...
    def test_inventory_identity_reaches_128_events_and_relation_links(self):
        data = self.data(); events = app.build_events(data)
        self.assertTrue(all(row["questionInventoryCorrection"] == data["inventoryCorrection"] for row in events))
        self.assertTrue(all(row["recoveryWrapperVersion"] == app.VERSION for row in events))
    ...
    def test_inventory_report_or_event_identity_cannot_be_reclassified(self):
        data, formal, source, answers, grades, sr, gr, ar, _ = deepcopy(self.fixture)
        data["report"]["recoveryEvidence"]["inventoryCorrection"]["version"] = "changed"
        with self.assertRaisesRegex(ValueError, "not_reproducible"):
            app.revalidate_report(formal, source, answers, grades, data["report"], sr, gr, ar)
        with self.assertRaisesRegex(ValueError, "inventory_event_lineage_mismatch"):
            app.build_events(data)
