...
from input_execution_v1.test_local_question_transport_inventory_v2 import install_synthetic_correction
...
class LocalQuestionEvaluationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.collection_receipt.update(inventoryCorrection=self.identity, runnerValidatorVersion=runner.VERSION)
        self.receipt_path.write_bytes(ev.packed(self.collection_receipt))
    ...
    def test_changed_local_code_or_protocol_is_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "freeze_code_hash_changed"):
            self.read()
    ...
    def test_inventory_formal_loader_preserves_bundle_and_adds_validated_lineage(self):
        result = local.load_formal(self.folder, self.root)
        self.assertEqual(result["inventoryCorrection"], self.identity)
        self.assertEqual(ev.packed(result["bundle"]), ev.packed(self.bundle))
        self.assertTrue(all(ref in result["evidence"] for ref in local.local_transport.correction_refs(self.identity)))

    def test_wrong_collection_inventory_identity_is_rejected(self):
        self.collection_receipt["inventoryCorrection"] = {"changed": True}
        self.receipt_path.write_bytes(ev.packed(self.collection_receipt))
        with self.assertRaisesRegex(ValueError, "inventory_collection_lineage_mismatch"):
            self.freeze()
        self.assertFalse((self.folder / "answers-freeze.json").exists())

    def test_changed_inventory_adapter_or_frozen_correction_is_rejected(self):
        self.freeze()
        receipt = self.folder / "answers-freeze.json"
        before = receipt.read_bytes()
        self.rewrite(receipt, lambda v: v["inventoryCorrection"].update(version="changed"))
        with self.assertRaisesRegex(ValueError, "freeze_contract"):
            self.read()
...
class LocalTransportIntegrationTests(unittest.TestCase):
    def test_actual_export_collect_validation_freeze_and_read_with_synthetic_native_boundary(self):
        from input_execution_v1 import local_question_transport_inventory_v2 as transport
        ...
        native = {"contexts": contexts, "summary": {"syntheticOnly": True, "modelCalls": 0, "version": runner.VERSION, "inventoryCorrection": f.identity},
