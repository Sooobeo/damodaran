"""Preserve nine v4 contexts; one fresh v4 context for each of 55 pending packets.
...
from input_execution_v1.local_question_runner_battery_v4 import (
    BASE, MODEL, PROFILE, resources, require, packed, sha, read, rooted,
    ...
    installation, resource_reason, system_state, battery_reason, PowerRequest,
)
VERSION = 'input-execution-v1-local-question-runner-partial-recovery-v5'
COHORT_VERSION = 'input-execution-v1-local-question-partial-recovery-cohort-v5'
PARTIAL_VERSION = 'input-execution-v1-local-question-partial-recovery-freeze-v5'
PARTIAL_FREEZE = 'content/model-comparison/input-execution-v1/local-question-partial-recovery-freeze-v5.json'
PRIOR_RUN = v4.DESTINATION
DESTINATION = BASE + '/s5-local-question-native-partial-recovery-v5-20260914/attempt-001'
PARTIAL_CLAIM = BASE + '/.local-question-native-partial-recovery-v5-generation.claim.json'
PRIOR_REVIEW = '.training/verifications/question-battery-partial-stop-audit-20260914.json'
...
PRIOR_PINS = {
    PRIOR_RUN + '/summary.json': '5e7597c17dcfe09bf08444709f87242a9053be4ea7a150b9a633696cf13ccf11',
    PRIOR_RUN + '/plan.json': '11290a7cd2c1ce5a0a600c210220e019fc12f02d668d2d790048b4911a3d8f20',
    v4.RECOVERY_CLAIM: '66a81ce592920d3fa1ca79fb97c681128f0724bf16c1479cf746ea4ccd9ac36a',
    v4.SUPERSESSION_PATH: '44b67e72e5b817c98d5a2968ca0bbadff86973e81b588d44102ef1aea478c720',
    PRIOR_REVIEW: '0023260da9900bce7090bd64c3b637ddeb8c4d59463f5a161de3f7849a857eca',
}
BATTERY_FREEZE_SHA = '9d883cbafa82b57a5bcda372d807dbce5ffbc92c74f3a7ac9a1c9724335fa533'
REQUIRED_PARTIAL_FILES = frozenset({
    'content/model-comparison/input-execution-v1/LOCAL_QUESTION_PARTIAL_RECOVERY_V5.md',
    *('scripts/model-comparison/input_execution_v1/' + name + '.py' for name in (
        'local_question_runner_partial_recovery_v5', 'test_local_question_runner_partial_recovery_v5',
        'local_question_transport_partial_recovery_v5', 'test_local_question_transport_partial_recovery_v5',
        'local_question_evaluation_partial_recovery_v5', 'test_local_question_evaluation_partial_recovery_v5',
        'local_question_append_evidence_partial_recovery_v5', 'test_local_question_append_evidence_partial_recovery_v5',
    )),
})
...
def prior_snapshot(root=ROOT):
    root = Path(root).resolve()
    folder = development(root, PRIOR_RUN)
    refs = [{'path': path, 'sha256': digest} for path, digest in PRIOR_PINS.items()]
    verify_refs(root, refs)
    state, evidence = validated_snapshot(folder, root)
    plan, _ = read(folder / 'plan.json')
    battery = v4.load_battery(root)
    require(battery['batteryFreeze']['sha256'] == BATTERY_FREEZE_SHA, 'prior_battery_freeze_changed')
    require(state['version'] == plan['version'] == v4.VERSION and state['contextProducerVersion'] == v4.VERSION
            and state['status'] == 'failed' and state['failure'] == {'type': 'ValueError', 'code': 'question_context_failed'}
            and type(state['expectedContexts']) is int and state['expectedContexts'] == 64
            and all(type(state[k]) is int and state[k] == 9 for k in ('completedContexts', 'freshNativeProcesses', 'completionRequestsSent'))
            and state['nativeStopped'] is True and state['powerRequest']['released'] is True
            and state['batteryExecution'] == plan['batteryExecution'] == battery, 'prior_partial_run_binding')
    require({p.name for p in folder.iterdir() if p.is_dir()} == set((*RETAINED_IDS, 'R010')), 'prior_context_inventory')
    failed = folder / 'R010'
    require(sorted(p.relative_to(failed).as_posix() for p in failed.rglob('*')) == ['input-packet.json', 'plan.json', 'preflight.json', 'summary.json'],
            'failed_context_no_native_or_request_required')
    failure, failed_refs = validated_snapshot(failed, root)
    failed_plan, _ = read(failed / 'plan.json')
    require(all(failure.get(k) == value for k, value in failed_plan.items())
            and failure['version'] == v4.VERSION and failure['reviewId'] == 'R010' and failure['status'] == 'failed'
            and failure['failure'] == {'type': 'ResourceGuardError', 'code': 'insufficient_start_availablephysicalbytes'}
            and type(failure['completionRequestsSent']) is int and failure['completionRequestsSent'] == 0
            and type(failure['previousModelRequests']) is int and failure['previousModelRequests'] == 0
            and failure['shutdown'] == {'processCreated': False, 'stopped': True}
            and failure['monitorError'] is None and failure['validatedDraft'] is None
            and failed_plan['profile'] == PROFILE and failed_plan['sampling'] == contract.NATIVE_SAMPLING
            and failed_plan['modelSha256'] == contract.MODEL_SHA and failed_plan['sourceExposure'] is False,
            'failed_context_zero_call_binding')
    observations, _ = read(failed / 'preflight.json')
    require(len(observations) == 2 and observations[1]['monotonicSeconds'] - observations[0]['monotonicSeconds'] >= 3,
            'prior_failed_preflight_pair')
    require(observations[0]['system']['availablePhysicalBytes'] < PROFILE['minimumStartPhysicalBytes']
            and observations[1]['system']['availablePhysicalBytes'] >= PROFILE['minimumStartPhysicalBytes'], 'prior_transient_memory_evidence')
    export_meta = {'batteryExecution': battery, 'zeroCallRecovery': battery['zeroCallRecovery'], 'evidence': [plan['exportManifest']]}
    claim = v4.validate_admission(folder, plan, state, export_meta, root)
    verify_refs(root, refs + evidence)
    require(validated_snapshot(folder, root)[0] == state, 'prior_changed_during_snapshot')
    return {'summary': state, 'plan': plan, 'battery': battery, 'claim': claim, 'evidence': evidence,
            'failedContextEvidence': failed_refs}


def partial_freeze_inputs(root=ROOT):
    root = Path(root).resolve()
    prior = prior_snapshot(root)
    return {'batteryExecution': prior['battery'],
            'priorRun': {'path': PRIOR_RUN, 'status': 'failed', 'expectedContexts': 64, 'completedContexts': 9,
                         'freshNativeProcesses': 9, 'completionRequestsSent': 9, 'completedReviewIds': list(RETAINED_IDS),
                         'summary': reference(root, PRIOR_RUN + '/summary.json'), 'plan': reference(root, PRIOR_RUN + '/plan.json'),
                         'artifactInventorySha256': sha(packed(prior['summary']['artifactFiles'])),
                         'failedContext': {'reviewId': 'R010', 'completionRequestsSent': 0, 'freshNativeProcesses': 0,
                                           'files': prior['failedContextEvidence'], 'nativeStoppedMeaning': 'no_process_was_created'}},
            'priorClaim': reference(root, v4.RECOVERY_CLAIM), 'priorSupersession': reference(root, v4.SUPERSESSION_PATH),
            'priorReview': reference(root, PRIOR_REVIEW), 'newRunPath': DESTINATION, 'newClaimPath': PARTIAL_CLAIM,
            'expectedReviewIds': list(contract.IDS), 'retainedReviewIds': list(RETAINED_IDS), 'pendingReviewIds': list(PENDING_IDS),
            'waitPolicy': WAIT_POLICY}


def partial_recovery_refs(identity):
    refs = [*v4.battery_refs(identity['batteryExecution']), identity['priorRun']['summary'], identity['priorRun']['plan'],
...
def validate_plan(folder, plan, export, install, template, metadata, version, ids, root):
    require(plan['version'] == version and plan['contextProducerVersion'] == v4.VERSION
...
def validate_prior(export_dir, root=ROOT):
    root = Path(root).resolve()
    prior = prior_snapshot(root)
    export = v4.load_export(export_dir, root)
    install = installation(root)
    template, metadata = gguf_template(rooted(root, MODEL))
    folder = rooted(root, PRIOR_RUN)
    validate_plan(folder, prior['plan'], export, install, template, metadata, v4.VERSION, contract.IDS, root)
    contexts = replay_contexts(folder, export['packets'][:9], export, template, metadata, root,
                               datetime.fromisoformat(prior['claim']['at']).timestamp())
    failed_packet = export['packets'][9]
    require((folder / 'R010/input-packet.json').read_bytes() == packed(failed_packet), 'failed_context_packet_changed')
    verify_refs(root, prior['evidence'] + export['evidence'])
    check_installation_stat(install, root)
    require(prior_snapshot(root)['summary'] == prior['summary'], 'prior_completed_context_changed')
    return {**prior, 'contexts': contexts, 'export': export, 'installation': install, 'template': template, 'metadata': metadata}
...
def replay_contexts(folder, packets, export, template, metadata, root, after):
    ...
    for packet in packets:
        ...
        require(context_plan.get('version') == state.get('version') == v4.VERSION, 'frozen_context_producer_identity')
        ...
        contexts.append({'reviewId': rid, 'contextId': cid, 'packetSha256': sha(packed(packet)), 'draft': draft,
                         'process': process, 'evidence': context_evidence, 'contextProducerVersion': v4.VERSION})
...
def run(export_dir, destination, root=ROOT):
    ...
    started = time.monotonic()
    completed, waits, failure, power, claim_ref = [], [], None, None, None
    admitted = False
    ...
    try:
        with resources.ExperimentLock():
            ...
            with PowerRequest() as power:
                ...
                plan = {'version': VERSION, 'contextProducerVersion': v4.VERSION, 'expectedContexts': 55,
                ...
                write_json(rooted(root, PARTIAL_CLAIM), {'version': VERSION, 'runPath': DESTINATION,
                           ...
                           'plan': reference(root, destination / 'plan.json'), 'admission': plan['admission'], 'at': utc()})
                ...
                for packet in export['packets'][9:]:
                    ...
                    execution_export = {**export, 'evidence': [*export['evidence'], claim_ref, wait['file']]}
                    verify_refs(root, execution_export['evidence'])
                    require(time.monotonic() - started < PROFILE['totalSeconds'], 'total_time_limit')
                    # The immutable v4 context repeats its own fresh preflight.
                    # Any failure from this call ends the run, never retries it.
                    completed.append(v4.execute_context(packet, destination / packet['reviewId'], install, template, metadata, execution_export, root, started))
                    print(json.dumps({'reviewId': packet['reviewId'], 'completed': 9 + len(completed), 'total': 64, 'reused': 9}), flush=True)
        verify_refs(root, [*export['evidence'], claim_ref, *[row['file'] for row in waits]])
    ...
    summary = {'version': VERSION, 'contextProducerVersion': v4.VERSION, 'status': 'completed' if failure is None else 'failed',
...
def validate_new_admission(folder, plan, summary, export, root):
    ...
    require(claim_raw == packed(claim) and summary['partialClaim'] == reference(root, PARTIAL_CLAIM)
            and set(claim) == {'version', 'runPath', 'partialRecoveryFreeze', 'priorRunSummary', 'priorClaim', 'exportManifest', 'plan', 'admission', 'at'}
    ...
    return claim, [admission['initialWait'], admission['poweredWait']]
...
def combine_contexts(retained, fresh):
    ...
    require([row['reviewId'] for row in retained] == list(RETAINED_IDS)
            ...
            and all(row['contextProducerVersion'] == v4.VERSION for row in (*retained, *fresh)), 'logical_cohort_partition_identity')
    contexts = []
...
def validate_run(run_dir, export_dir, root=ROOT):
    ...
    require(summary['version'] == VERSION and summary['status'] == 'completed' and summary['failure'] is None
            ...
            and summary['contextProducerVersion'] == v4.VERSION and summary['nativeStopped'] is True
    ...
    logical = {'version': COHORT_VERSION, 'status': 'completed_logical_cohort', 'expectedContexts': 64, 'completedContexts': 64,
               ...
               'contextProducerVersion': v4.VERSION, 'sourceRuns': source_runs, 'nativeStopped': True,
               'sourceExposure': False, 'actualCollaborationSpawns': 0, 'modelSha256': contract.MODEL_SHA,
