"""Explicit battery-v4 answer freeze/report over unchanged recovery-v2 packets.
...
from input_execution_v1 import local_question_evaluation_recovery_v3 as baseline
from input_execution_v1 import recovery_evaluation_v2 as recovery

VERSION = "input-execution-v1-local-question-evaluation-battery-v4"
...
def load_formal(folder, root=ROOT):
    """Keep frozen v1/v2/v3 provenance and bind the authorized battery execution."""
    root = Path(root).resolve()
    formal = baseline.load_formal(local_transport.private_path(root, folder), root)
    battery = local_transport.load_battery(root)
    ev.require(battery["zeroCallRecovery"] == formal["zeroCallRecovery"], "battery_recovery_lineage_mismatch")
    formal["batteryExecution"] = battery
    formal["evidence"] = [*formal["evidence"], *local_transport.battery_refs(battery)]
...
def checked_collection(formal, directory, root):
    ...
    ev.require(receipt.get("batteryExecution") == formal["batteryExecution"] and
               receipt.get("contextProducerVersion") == local_transport.RUNNER_VERSION, "battery_collection_lineage_mismatch")
    rows = result["rows"]
...
def freeze_answers(folder, collection, root=ROOT):
    ...
    receipt = {"version": VERSION, "status": "local_native_answers_frozen",
        ...
        "batteryExecution": formal["batteryExecution"], "contextProducerVersion": local_transport.RUNNER_VERSION,
...
def report_recovery_evidence(formal, evidence, answers):
    ...
    return {**value, "wrapperVersion": VERSION, "inventoryCorrection": deepcopy(formal["inventoryCorrection"]),
            ...
            "contextProducerVersion": local_transport.RUNNER_VERSION}
