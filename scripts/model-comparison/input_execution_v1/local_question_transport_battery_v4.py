"""Explicit battery-v4 runner collection over byte-unchanged v1 exports.
...
VERSION = "input-execution-v1-local-question-transport-battery-v4"
RUNNER_VERSION = "input-execution-v1-local-question-runner-battery-v4"
...
def contents(formal, export, run, root):
    ...
    ev.require(result["summary"].get("batteryExecution") == battery and
               result["summary"].get("contextProducerVersion") == RUNNER_VERSION, "battery_runner_lineage_mismatch")
    ...
    for context in contexts:
        ev.require(context.get("contextProducerVersion") == RUNNER_VERSION, "battery_context_producer_mismatch")
        ...
        bindings.append({"reviewId": rid, "actorId": answer["reviewer"]["actorId"],
                         ...
                         "contextProducerVersion": context["contextProducerVersion"]})
    refs = [*formal["evidence"], *export_refs, *result["evidence"], *correction_refs(identity), *recovery_refs(zero_call), *battery_refs(battery)]
...
def receipt_for(formal, built, export, run, when, root):
    return {"version": VERSION, "status": "local_question_collected_not_frozen", "recordedAtUtc": when,
            ...
            "contextProducerVersion": RUNNER_VERSION,
