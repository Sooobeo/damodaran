"""Explicit inventory-v2 answer freeze/report over unchanged recovery-v2 packets.
...
from input_execution_v1 import local_question_transport_inventory_v2 as local_transport
from input_execution_v1 import local_question_evaluation_v1 as baseline
from input_execution_v1 import recovery_evaluation_v2 as recovery

VERSION = "input-execution-v1-local-question-evaluation-inventory-v2"
...
def load_formal(folder, root=ROOT):
    """Keep frozen formal bytes and v1 provenance; bind the explicit correction."""
    root = Path(root).resolve()
    formal = baseline.load_formal(local_transport.private_path(root, folder), root)
    identity = local_transport.load_correction(root)
    formal["inventoryCorrection"] = identity
    formal["evidence"] = [*formal["evidence"], *local_transport.correction_refs(identity)]
...
def freeze_answers(folder, collection, root=ROOT):
    ...
    receipt = {"version": VERSION, "status": "local_native_answers_frozen",
        ...
        "inventoryCorrection": formal["inventoryCorrection"],
...
def read_frozen_answers(formal, root):
    ...
    expected = {"version": VERSION, "status": "local_native_answers_frozen", "answerContexts": 64,
        ...
        "questionCodeFiles": formal["questionCodeFiles"], "inventoryCorrection": formal["inventoryCorrection"],
