"""Focused v5 freeze/provenance/report tests; synthetic completed validator data."""
...
class PartialEvaluationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.bindings = [{"reviewId": rid, "contextId": "synthetic-partial-" + rid,
            "contextProducerVersion": transport.CONTEXT_VERSION, "sourceRun": source_run(i, self.runs)} for i, rid in enumerate(tr.IDS)]
    ...
    def test_changed_partial_identity_or_origin_is_rejected_before_freeze(self):
        ...
        for kind in ("identity", "origin", "single-run"):
    ...
    def test_actual_v5_transport_to_freeze_keeps_answer_content_and_9_55_join(self):
        f = self.f; contexts = []
