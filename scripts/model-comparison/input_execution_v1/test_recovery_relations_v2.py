...
from input_execution_v1 import recovery_relations_v2 as r
...
class RecoveryRelationsV2Tests(unittest.TestCase):
    def test_writer_binds_v2_receipt_and_original_evaluation_lineage(self):
        from input_execution_v1 import recovery_cohort_v2 as cohort
        loaded = {"rows": rows(), "manifest": {"validationVersion": r.lineage.VALIDATION_VERSION,
                  "validationLineage": {"syntheticOnly": True}}, "evidence": {"files": []}}
        lineage = {"syntheticOnly": True, "priorFrozenFilesModified": False}
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); destination = root / r.BASE / "synthetic-report"
            with patch.object(cohort, "validate_cohort", return_value=loaded) as validator, \
                 patch.object(r.lineage, "verify", return_value=lineage):
                manifest = r.write_report("synthetic-cohort", destination, root)
            report = r.e.read_json(destination / "relation-report.json")
            self.assertEqual(validator.call_count, 2)
            self.assertEqual(report["cohortManifest"], loaded["manifest"])
            self.assertEqual(report["evaluationLineage"], lineage)
            self.assertEqual(report["cohortValidatorCodeSha256"], r.e.sha(Path(cohort.__file__).read_bytes()))
            self.assertEqual(manifest["reportSha256"], r.e.sha((destination / "relation-report.json").read_bytes()))
            self.assertEqual(report["observations"], r.v1.diagnose_rows(loaded["rows"])["observations"])

    def test_writer_refuses_v1_validation_before_creating_any_report(self):
        from input_execution_v1 import recovery_cohort_v2 as cohort
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); destination = root / r.BASE / "synthetic-report"
            with patch.object(cohort, "validate_cohort", return_value={"manifest": {}}), \
                 patch.object(r.lineage, "verify", return_value={"syntheticOnly": True}):
                with self.assertRaisesRegex(ValueError, "validator_v2_required"):
                    r.write_report("synthetic-cohort", destination, root)
            self.assertFalse(destination.exists())

    def test_writer_refuses_a_changed_cohort_between_validations(self):
        from input_execution_v1 import recovery_cohort_v2 as cohort
        loaded = {"rows": rows(), "manifest": {"validationVersion": r.lineage.VALIDATION_VERSION}, "evidence": {"files": []}}
        changed = deepcopy(loaded); changed["evidence"]["changed"] = True
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp); destination = root / r.BASE / "synthetic-report"
            with patch.object(cohort, "validate_cohort", side_effect=[loaded, changed]), \
                 patch.object(r.lineage, "verify", return_value={"syntheticOnly": True}):
                with self.assertRaisesRegex(ValueError, "changed_during_diagnosis"):
                    r.write_report("synthetic-cohort", destination, root)
            self.assertFalse(destination.exists())
