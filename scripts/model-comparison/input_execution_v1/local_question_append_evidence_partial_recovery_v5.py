"""Prepare partial-recovery-v5 local QA observations; append only with --append.
...
from input_execution_v1 import local_question_transport_partial_recovery_v5 as local_transport
from input_execution_v1 import evaluation as ev
from input_execution_v1 import local_question_evaluation_partial_recovery_v5 as reval
from input_execution_v1 import recovery_relations_v2 as relations

VERSION = "input-execution-v1-local-question-ledger-evidence-partial-recovery-v5"
ledger = original.ledger
CODE_PATH = "scripts/model-comparison/input_execution_v1/local_question_append_evidence_partial_recovery_v5.py"
...
def revalidate_report(formal, source_rows, answers, grade_rows, report, source_ref, grades_ref, answer_refs, question_provenance):
    ...
    expected["recoveryEvidence"] = reval.report_recovery_evidence(
        formal, [*formal["evidence"], source_ref, grades_ref, *answer_refs], answers, question_provenance)
...
def validate_final_evidence(*, folder, cohort, source_reviews, grades, report_path, relation_folder, root=ROOT):
    ...
    answers, answer_refs, question_provenance = reval.read_frozen_answers_with_provenance(formal, graph.root)
    ...
    revalidate_report(formal, source_rows, answers, grade_rows, report,
                      simple_ref(graph, source_reviews), simple_ref(graph, grades), answer_refs, question_provenance)
    ...
    return {"graph": graph, "bundle": formal["bundle"], "report": report, "outputs": formal["cohort"]["rows"],
            ...
            "partialRecovery": deepcopy(formal["partialRecovery"]), "questionProvenance": question_provenance}
...
def build_events(validated):
    ...
    battery = validated["batteryExecution"]
    partial, question_provenance = validated["partialRecovery"], validated["questionProvenance"]
    ev.require(partial["batteryExecution"] == battery and report["recoveryEvidence"]["partialRecovery"] == partial and
               report["recoveryEvidence"]["questionProvenance"] == question_provenance and
               report["recoveryEvidence"]["coordinatorVersion"] == local_transport.RUNNER_VERSION, "partial_event_lineage_mismatch")
    question_contexts = local_transport.validate_question_provenance(question_provenance, partial)
    ev.require(report["recoveryEvidence"]["batteryExecution"] == battery and
               battery["zeroCallRecovery"] == zero_call and
               report["recoveryEvidence"]["contextProducerVersion"] == local_transport.CONTEXT_VERSION,
               "battery_event_lineage_mismatch")
    ...
    for event in ev.ledger_observations(bundle, report, validated["evidenceRefs"]):
        ...
        event["batteryExecution"] = deepcopy(battery)
        event["questionContextProducerVersion"] = local_transport.CONTEXT_VERSION
        event["questionCoordinatorVersion"] = local_transport.RUNNER_VERSION
        event["partialRecovery"] = deepcopy(partial)
        event["questionSourceRuns"] = deepcopy(question_provenance["sourceRuns"])
        event["questionContext"] = deepcopy(question_contexts[event["reviewId"]])
        inputs.append(event)
    ...
    for row in observations:
        ...
        metadata["batteryExecution"] = deepcopy(battery)
        metadata["questionContextProducerVersion"] = local_transport.CONTEXT_VERSION
        metadata["questionCoordinatorVersion"] = local_transport.RUNNER_VERSION
        metadata["partialRecovery"] = deepcopy(partial)
        metadata["questionSourceRuns"] = deepcopy(question_provenance["sourceRuns"])
        metadata["questionContext"] = deepcopy(question_contexts[mapped["reviewId"]])
...
def prepare(*, folder, cohort, source_reviews, grades, report_path, relation_folder, destination, append=False, root=ROOT):
    ...
    result = {"version": VERSION, "status": "prepared", "createdAtUtc": datetime.now(timezone.utc).isoformat(),
        ...
        "recovery": validated["recovery"], "inventoryCorrection": validated["inventoryCorrection"], "zeroCallRecovery": validated["zeroCallRecovery"],
        "batteryExecution": validated["batteryExecution"], "questionContextProducerVersion": local_transport.CONTEXT_VERSION,
        "partialRecovery": validated["partialRecovery"], "questionProvenance": validated["questionProvenance"],
        "questionCoordinatorVersion": local_transport.RUNNER_VERSION, "humanReviewed": False, "modelCalls": 0, "translationReviewEventsAdded": 0,
