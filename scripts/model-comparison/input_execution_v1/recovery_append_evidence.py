"""Prepare explicit recovery S5/relation observations; append only with --append.
...
VERSION = "input-execution-v1-recovery-ledger-evidence-v1"
ledger = original.ledger
CODE_PATH = "scripts/model-comparison/input_execution_v1/recovery_append_evidence.py"
RELATION_CODE = "scripts/model-comparison/input_execution_v1/recovery_relations.py"
COHORT_CODE = "scripts/model-comparison/input_execution_v1/recovery_cohort.py"
...
def revalidate_report(formal, source_rows, answers, grade_rows, report, source_ref, grades_ref, answer_refs):
    ...
    expected["recoveryEvidence"] = {"wrapperVersion": reval.VERSION, "formalManifestSha256": formal["manifestSha256"],
                                    **formal["recovery"],
                                    "evidenceFiles": [*formal["evidence"], source_ref, grades_ref, *answer_refs],
...
def validate_final_evidence(*, folder, cohort, source_reviews, grades, report_path, relation_folder, root=ROOT):
    ...
    source_reviews, grades, report_path, relation_folder = (
        transport.private_path(graph.root, path) for path in (source_reviews, grades, report_path, relation_folder))
    ...
    return {"graph": graph, "bundle": formal["bundle"], "report": report, "outputs": formal["cohort"]["rows"],
            ...
            "recovery": deepcopy(formal["recovery"])}
...
def prepare(*, folder, cohort, source_reviews, grades, report_path, relation_folder, destination, append=False, root=ROOT):
    ...
    result = {"version": VERSION, "status": "prepared", "createdAtUtc": datetime.now(timezone.utc).isoformat(),
        ...
        "recovery": validated["recovery"], "humanReviewed": False, "modelCalls": 0, "translationReviewEventsAdded": 0,
