"""Explicit zero-call recovery-v3 answer freeze/report over unchanged recovery-v2 packets.
...
from input_execution_v1 import local_question_evaluation_inventory_v2 as baseline
from input_execution_v1 import recovery_evaluation_v2 as recovery

VERSION = "input-execution-v1-local-question-evaluation-zero-call-recovery-v3"
...
def load_formal(folder, root=ROOT):
    """Keep v1/v2 formal provenance and bind the validated zero-call recovery."""
    root = Path(root).resolve()
    formal = baseline.load_formal(local_transport.private_path(root, folder), root)
    zero_call = local_transport.load_recovery(root)
    formal["zeroCallRecovery"] = zero_call
    formal["evidence"] = [*formal["evidence"], *local_transport.recovery_refs(zero_call)]
