"""Prepare battery-v4 local QA observations; append only with --append.
...
from input_execution_v1 import local_question_transport_battery_v4 as local_transport
from input_execution_v1 import evaluation as ev
from input_execution_v1 import local_question_evaluation_battery_v4 as reval
from input_execution_v1 import recovery_relations_v2 as relations

VERSION = "input-execution-v1-local-question-ledger-evidence-battery-v4"
ledger = original.ledger
CODE_PATH = "scripts/model-comparison/input_execution_v1/local_question_append_evidence_battery_v4.py"
...
def build_events(validated):
    ...
    ev.require(report["recoveryEvidence"]["batteryExecution"] == battery and
               ...
               report["recoveryEvidence"]["contextProducerVersion"] == local_transport.RUNNER_VERSION,
    ...
    for event in ev.ledger_observations(bundle, report, validated["evidenceRefs"]):
        ...
        event["questionContextProducerVersion"] = local_transport.RUNNER_VERSION
        inputs.append(event)
    ...
    for row in observations:
        ...
        metadata["questionContextProducerVersion"] = local_transport.RUNNER_VERSION
...
def prepare(*, folder, cohort, source_reviews, grades, report_path, relation_folder, destination, append=False, root=ROOT):
    ...
    result = {"version": VERSION, "status": "prepared", "createdAtUtc": datetime.now(timezone.utc).isoformat(),
        ...
        "batteryExecution": validated["batteryExecution"], "questionContextProducerVersion": local_transport.RUNNER_VERSION, "humanReviewed": False, "modelCalls": 0, "translationReviewEventsAdded": 0,
