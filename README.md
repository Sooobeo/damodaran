...
Damodaran의 선별 자료로 공부하는 로컬 한국어 학습실이다. 자료실·8개 학습 단원·HTML/PDF 읽기·무료 로컬 한국어 번역·금융용어·Excel 가이드·메모와 학습 기록을 연결한다. 초기 설치 기본값은 Argos Translate이며, 이 PC의 현재 웹 운영 제공자도 속도·안정성을 위해 Argos다. 품질 비교와 격리 앱 검증을 마친 Hy-MT2 문맥 구성은 모델과 등록 기록을 보존한 선택형 제공자다. 두 제공자 모두 API 키가 필요 없다.
...
이 PC는 별도 품질 비교·등록·격리 앱 검증 후 Hy-MT2의 운영 HTML 3문단 생성·저장·캐시도 확인했다. 다만 CPU에서 긴 단일 HTML 블록을 처리할 때 지연과 절전 복귀 후 종료 문제가 확인되어, 2026-09-20 현재 `.env.local`은 `TRANSLATION_PROVIDER=argos`를 명시한다. Hy-MT2 모델·등록·기존 번역은 삭제하지 않았으며 필요할 때만 다시 선택한다. 설치·등록·실행 계약은 [로컬 Hy-MT2 안내](scripts/local-hymt/README.md)를 따른다.
