...
**2026-09-13 다음 작업 기준은 [인수인계28절](TRANSLATION_HANDOFF_20260911.md#28-공통-입력-처리와-의미-관계-검사-개선-실행-계획)이다.** 구조 문맥·문맥별 용어 사전·수량/논리 관계 검사를 먼저 준비하고 Hy7 네 구성의 효과와 일반 회귀를 비교한다. TG27 읽기6은 후순위 미완료로 보존한다. 이번 변경은 실행 계획 문서화이며 새 구현·모델 실행·등록 변경은 없다. 아래27절 상태와 프로세스/RAM 관측은2026-09-12 당시 기록이며 실행 전에 현재 상태를 다시 확인한다.

2026-09-11~12 재개 후 품질 비교의 최신 상태는 [인수인계27절](TRANSLATION_HANDOFF_20260911.md#27-읽기6-중단-증거와-재시도-사전검사-보류)과 [오류 분석](content/model-comparison/ERROR_LEDGER_REPORT_20260911.md)에 기록한다. Qwen v5 진단은 첫 실제 미탐으로 중단했고 미등록 상태를 유지한다. TG27은 [개발 원문18개 대조](content/model-comparison/TG27_PARTIAL_REVIEW_20260912.md)에서 중요5·경미2·관측 오류 없음5·보류6을 기록했다. 기존 실패 실행의 완료16개와 [별도 재실행을 마친17·18번](content/model-comparison/TG27_TAIL_RECOVERY_20260912.md)을 구분하며 단일 개발18 성공으로 합치지 않는다. 공개 원문 읽기6은 저장 결과 없이 중단됐고 재시도는 시작 RAM 기준과 프로세스 조회를 충족하지 못해 보류됐다. 현재 모델 실행은 없다. 오류 원장은738사건이며 TG27의 개발18문단·용어92출현·기존 의미 규칙 관측16개를 실제 증거와 연결했다. 선택한 기존 개발 증거를 모델 호출 없이 다시 연결하는 명령은 아래와 같고, 새 진단의 명령은 [별도 안내](scripts/model-comparison/PARTIAL_TG27_REVIEW.md)를 따른다. 기존 개인 데이터·모델·검수 기록을 보존하며 전체 학습 수용 기준은 미충족이다.
