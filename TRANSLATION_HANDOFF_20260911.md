...
> **최신 작업 기준: 2026-09-13, 공통 입력 처리와 의미 관계 검사 개선을 우선한다. [28절](#28-공통-입력-처리와-의미-관계-검사-개선-실행-계획)을 먼저 읽는다.** 문서 구조를 반영한 문맥·문맥별 용어 사전·수량/논리 관계 검사를 준비하고 Hy-MT2 7B의 네 구성을 비교한다. TG27 읽기6 재시도는 후순위 미완료로 보존하며 새 개선의 선행조건이 아니다. 이번 갱신은 문서 작업이며 새 구현·모델 추론·앱 적용을 수행하지 않았다.
>
> 마지막 엔진 실측은 **2026-09-12의 27절**이다. 개발 원문18개 대조는 완료했지만 실패 실행의 완료16개와 새17·18번을 단일 성공 실행으로 합치지 않는다. 중요 오류5·경미2·관측 오류 없음5·보류6으로 수용 기준 미충족이다. 읽기6은 저장 결과 없이 중단됐고 재시도 사전검사는 보류됐다. 당시 프로세스 부재·RAM 값은 현재 실행 상태를 뜻하지 않으므로 실행 직전에 다시 확인한다. 1~27절의 시각별 기록은 당시 이력으로 보존한다.
>
...
아래 항목은 이번 문서 갱신 때 모두 **미착수**다. 읽기 감사 사실을 구현 완료 체크로 바꾸지 않는다. 이후 항목을 완료할 때만 체크하고 실제 산출물·검사·미완료를28.8에 추가한다.

- [ ] **S1 — 입력·평가 계약과 자료 고정.** 현재 Hy7 등록 정체성, 실제 입력 경로, 문서 구조의 가용 범위와 오류 유형을 대조한다. 새 개발 자료·사전 출처·문맥 선택·평가 명제/용어/질문·완료/중단 규칙을 추론 전에 고정한다. 기본 계획은 금융8·일반8의16개 학습 단위이며, 실제 문서의 구조를 가진 문단 묶음과 반대 의미의 최소대립 사례를 포함한다. 구체적 수·선정·평가 분모는 자료 감사를 마친 명세에 확정하고 결과를 본 뒤 늘리거나 줄이지 않는다. 이미 본 개발 오류는 회귀/진단 자료로 표시하고, 소비한 최종 test·개인 기록을 새 개발/학습 자료로 편입하지 않는다. 독립 평가 자료의 규모·선정 절차도 개발 결과를 보기 전에 정하고 내용은 선택 완료 때까지 분리한다.
- [ ] **S2 — 모델 없이 입력 준비 구현.** 새 문맥 선택기·의미 사전/선택기를 별도 코드·프로필로 구현한다. 원문 보존, 잘못된 버전/블록 거부, 문맥 출처, 중복 제거, 실제 토큰 예산, 사전 선택의 금융/일반 반례, 평가 주석 유출 방지를 확인한다. 같은 원문·구조 묶음에서 C0~C3의 실제 프롬프트와 차이를 검토하고 해시로 고정한다. 무거운 모델이 필요한 tokenizer 경로라면 해당 확인만 단일 슬롯에서 별도로 수행하고 번역 생성과 구분한다.
- [ ] **S3 — 저장 출력으로 관계 검사 검증.** 기존 원문/번역/판정 해시를 유지한 별도 진단에서 실제 오류와 정상 반례를 함께 검사한다. 원래 판정과 신규 경고를 연결하고 유형별 TP/FP/FN/TN·보류·미지원·경고 범위를 기록한다. 기존 원장에 구현된 추가 기록/중복 방지를 재사용하며 원장을 새로 만들거나 오류를 중복 가산하지 않는다. 출력 선택이 사후적이면 그 범위를 명시하고 독립 성능으로 표시하지 않는다.
- [ ] **S4 — Hy7 네 구성의 실제 생성.** 아래 C0~C3를 동일한 새 입력 집합에서 비교한다.16개로 확정하면 기본 생성 상한은64개 출력이다. 기존 동일 출력 재사용은 원문·문맥·프롬프트·모델/런타임·생성 설정과 처리 정체성이 모두 같은 경우만 허용하며 재사용 여부를 기록한다. 기능/프로토콜 실패는 생성 확대 전에 고치고 새 코드·실행 버전을 남긴다. 이미 완료한 출력의 재생성으로 유리한 결과를 고르지 않는다.
...
아래 경로는 **계획 위치이며 아직 생성하지 않았다.** 기존 hash에 묶인 `run_hymt.py`, 등록 코드·사전, TG27/Qwen 검증 도구를 직접 수정해서 새 실험을 연결하지 않는다. 새 도구 이름과 명령은 구현·검증 후 실제 안내에 추가하며, 존재하지 않는 CLI를 지금 실행 가능한 명령으로 쓰지 않는다.
...
**다음 구현 작업의 첫 행동은 S1의 입력·평가 계약 작성이다.** 현재 문서·관련 코드/등록 해시·git diff를 확인하고, 구조 문맥과 새 사전이 실제 모델 입력까지 전달되는 경로를 정한 뒤 S2/S3을 진행한다. 그 이후 현재 자원을 확인해 S4로 넘어간다. TG27 coordinator 실행이나 모델 다운로드부터 시작하지 않는다.
...
2026-09-13 이번 갱신 범위는 인수인계와 관련 문서의 계획·연결 정리다. 앱/도구 구현·모델 추론·가중치 학습·등록 변경·운영 DB 변경은 수행하지 않았다. S1~S6 산출물과 개선 수치는 아직 없으며 앱 테스트나 실제 엔진 검증을 새로 통과한 것으로 보고하지 않는다.
