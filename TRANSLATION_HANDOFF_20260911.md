...
**최신 S5 상태 — 2026-09-20 memory-v6 실행 시작, 기존 질문 9개 보존.** 현재 자원으로 시작하라는 사용자 요청에 따라 시작 physical만9→7GiB로 조정한 별도 실행 경로를 검증·동결했다. 나머지55개만 처리하며 실행 중 보호와 품질 기준은 유지한다. 실제 시작 조건·진행 결과·실행 명령은 [9월 20일 재개 기록](content/model-comparison/input-execution-v1/RESUME_20260920.md)을 따른다. 전체 64개 답변 동결·128개 질문 채점·후보 선택·조건부 S6은 아직 미완료다.
...
- [x] **S5 — 원문 대조·질문 평가·후보 선택.** 2026-09-20 완료. 같은 원문별 네 출력의 중요 의미 오류, 핵심 관계, 용어, 자연스러움, 일반 회귀를 대조했다. 한국어 전용 로컬 독립 답변64개를 먼저 동결한 뒤 다른 도우미가 원문 근거로128질문을 채점했다. 유형·노출·생산 버전·원9/새55 차이를 기록했으며 원문을 본 root가 질문 답변을 대신 생성하지 않았다. 같은 관계 검사와 고정 선택 기준을 적용한 [결과](content/model-comparison/input-execution-v1/RESULT_20260920.md)는 적격 후보 없음이다.
- [x] **S6 — 유효한 후보만 확대 여부 결정.** 2026-09-20 기존 등록 유지로 종결했다. C1·C3의 일반 질문 회귀와 C2의 필수 개선 부재로 확대 대상이 없으므로 Hy30 비교·독립80단위 평가·새 배포/등록·격리 앱 검증은 시작하지 않았다. 이 표시는 독립 평가 통과를 뜻하지 않는다. 가중치 학습·선택 재번역이나 새 후보 개선은 별도 실험으로 정하며, 같은 평가를 통과하도록 기준을 완화하거나 재조정하지 않는다.
...
**최신 S5 상태 — 2026-09-20 평가 완료, 기존 등록 유지.** 원9개와 memory-v6 새55개의 실제 질문 답변을 모두 수집·동결했고128질문을 채점했다(정답117·오답11·미해결0). 원문 검토64개·명제192개·용어156개를 포함한 정식 보고서와 독립 집계 대조를 완료했다. C1은 일반 질문 회귀·새 핵심 명제 손상, C2는 필수 개선 부재, C3는 일반 질문 회귀로 탈락했다. `retain_registered_configuration`에 따라 S6 확대평가·신규 등록 없이 기존 구성을 유지한다. 새 모델 품질 합격이나 사람 학습 효과로 해석하지 않는다. [개발 비교 결과](content/model-comparison/input-execution-v1/RESULT_20260920.md)와 [재개·pause 이력](content/model-comparison/input-execution-v1/RESUME_20260920.md)을 따른다. 원장128관측 추가·836→964사건·기존 바이트와 번역 검토146개 보존을 실제 영수증으로 확인했다.

아래 S4~배터리 v4 시작 설명은9월14일 당시 이력이며, 질문·채점의 현재 상태는 위9월20일 완료 기록을 따른다.
