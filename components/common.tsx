...
export function JobCard({ job }: { job: Job }) {
  ...
  return <div className="job-card"><div className="row-between"><strong>{label(job.type)}</strong><span className={`tag ${job.status === 'failed' ? 'red' : active ? 'blue' : ''}`}>{job.status === 'completed' ? '작업 완료' : label(job.status)}</span></div><small>작업 {job.id.slice(0, 8)} · 전체 {job.total}개</small><div className="progress-track"><span style={{ width: `${job.total ? Math.min(100, ((job.completed + job.failed + job.needsReview) / job.total) * 100) : 0}%` }}/></div><p className="job-stats">완료 {job.completed} · 검토 {job.needsReview} · 실패 {job.failed} · 남음 {job.remaining}</p>{job.errorMessage && <p className="inline-error">{job.errorMessage}</p>}<div className="job-actions">{job.resourceId && <Link href={`/resources/${job.resourceId}`} className="text-link">자료 보기 <ChevronRight size={14}/></Link>}{active ? <button className="button small secondary" disabled={busy} onClick={() => action('cancel')}><X size={14}/> 이 작업 전체 취소</button> : ['failed', 'partial', 'cancelled'].includes(job.status) && <button className="button small secondary" disabled={busy} onClick={() => action('retry')}><RefreshCw size={14}/> 미완료 재시도</button>}</div>{active && <small>{job.type === 'quality' ? '의미 검사 취소는 저장된 번역을 삭제하지 않습니다.' : '취소해도 이미 전송된 번역 호출의 사용량은 발생할 수 있습니다.'}</small>}</div>;
}
