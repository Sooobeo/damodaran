# 달모다란 (MoonModaran) 개인 학습실 — Codex 구현 설계서

작성일: 2026-09-09  
문서 버전: 1.3
제품 가칭: 달모다란 (MoonModaran)
...
- 무료로 공개된 번역 도구를 앱에 통합한다. 초기 설치 기본 번역은 Argos Translate의 로컬 영어→한국어 모델 1.1이며 API 키를 요구하지 않는다. 기존 OpenAI 기본안은 이 최신 요구로 대체한다.
- 최신 추가 승인으로 공개 영한 번역 모델의 실제 가중치를 금융 문장으로 미세조정하고 전후를 평가한다. 단순 용어 치환이나 검수 번역 저장으로 이 요구를 대신하지 않는다. 학습 결과의 앱 적용은 평가와 추론 검증을 거쳐 판단하며 설정 생략 시 초기 Argos 기본값은 유지한다. 이 PC의 별도 Hy-MT2 명시 선택·실제 검증 상태는 구현 상태 문서에 구분한다.
- 후속 승인으로 별도 사람 검수 자료가 없는 상태에서 도우미가 저장된 실제 영문 자료를 번역하여 새 학습쌍을 만든다. 원문 번역과 도우미 독자 작성 문장을 구분하고 모두 사람 미검수로 표시한다. 기존 실험·최종 시험은 보존하며, 새 실험은 문서 단위 분할과 숫자·단위·수식 검사, 독립 의미 대조를 사용한다.
- 최신 목표는 금융 용어 표기 적중 90% 이상이다. 금융·일반 문맥의 다의어를 구분한 새 학습 예문과 독립 평가 자료를 만들고, 기존 앱 사전의 기준 표기를 학습 전에 고정한다. 원시 모델의 용어 적중과 앱 사후 용어 보정 효과를 구분한다. 일반 문맥에 금융 뜻을 주입하거나 숫자·일반 번역을 악화시키면서 목표를 달성한 것으로 처리하지 않는다.
- 후속 요구는 무료 로컬 모델만으로 현재 PC에서 번역 전체의 품질을 최대한 높이는 것이다. 용어 90%는 중간 목표이며 원문 의미·주체와 관계·부정·조건·단위·수식·누락/추가·한국어 자연스러움을 함께 비교한다. v5의 고정 데이터와 판정은 보존하고, 모델명을 가린 원문 대조 결과 및 실제 문단의 문맥 처리도 별도로 확인한다. 더 큰 공개 모델도 실제 비교 대상으로 검토하며 이번 개선에서 유료 온라인 모델은 사용하지 않는다. 도우미의 평가를 전문가 검수나 모든 문서의 무오류 보장으로 표시하지 않는다.
- 추가 선택형 `hymt` 로컬 제공자는 비교에서 선택한 Hy-MT2 구성의 가중치·실행기·사전·의미 대조 근거를 등록한 경우에만 준비 완료로 표시한다. 문단 원문과 기존 참고 문맥을 전달하고, 원문을 숫자 보호 표식 사이 조각으로 나누거나 번역 후 용어를 강제 치환하지 않는다. 기존 원문 버전·검수 기록·개인 기록을 보존하며 전체 모델 정체성으로 캐시를 구분한다. 등록만으로 기본 제공자를 바꾸지 않으며 실제 앱 추론·저장·캐시를 별도 검증한다. 상세 계약은 SYSTEM_DESIGN.md 8.5와 `scripts/local-hymt/DEPLOYMENT.md`를 따른다.
...
| 초기 기본 번역 제공자 | 무료 공개 도구 Argos Translate, worker의 로컬 Python 하위 프로세스 |
...
- 새 PDF 가져오기·업로드는 PDF 전용 `pdf-paragraphs-v2`를 사용한다. 기존 `pdf-paragraphs-v1`·`structured-v2`와 HTML의 추출 규칙·설정 해시는 유지하며, 저장된 추출 대기 버전도 그 버전에 기록된 추출기로 처리한다.
- 추출 블록에 페이지 인덱스, 순서, 가능하면 정규화된 좌표와 추출 상태를 기록한다.
- 단일열 본문의 이어지는 줄은 방향·글꼴·들여쓰기·간격이 일관될 때만 문단으로 묶고, 독립 글머리·제목·푸터는 분리한다. 묶인 문단에는 원래 줄들의 좌표를 함께 남긴다. PDF.js가 숫자나 단어 중간을 나눈 경우 임의 공백을 넣지 않으며 소수·음수·백분율·원문 기호를 보존한다.
- v2는 미완결 글머리 도입 뒤에 같은 들여쓰기·글꼴·크기·간격의 안전한 하위 글머리 항목이 2개 이상 이어지면 도입과 전체 목록을 한 번역 단위로 묶는다. 지원하는 도입 경계는 콜론 또는 한정된 be 동사 형태이며, 제목처럼 보인다는 이유만으로 묶지 않는다. 항목 사이에는 줄바꿈만 넣고 원문 내용·기호·순서와 좌표 합집합을 보존한다. 뒤쪽 항목의 구조가 다르거나 전체 6,000자를 넘으면 앞부분만 묶지 않고 후보 전체를 원래 단위로 둔다.
- 단순 x/y 정렬만으로 다단 문서가 올바르게 읽힌다고 가정하지 않는다. 순서가 불명확하면 페이지 전체 원본을 함께 보게 한다.
- 표·여러 열·수식·회전 텍스트 또는 불충분한 글꼴·좌표 정보가 감지되면 문단 병합을 보류하고 원래 텍스트 조각과 원본 대조 경고를 유지한다. 이 처리는 표의 행·열 관계를 복원하는 기능이 아니며, 도표 라벨을 하나의 문장으로 합치지 않는다.
...
무료 로컬 번역의 전체 품질을 높이기 위해 추출된 문맥도 확인한다. 사용자가 같은 자료를 새로 가져올 때 파일 바이트가 같아도 추출기·설정이 달라졌으면 별도 원문 버전을 생성한다. 원본 파일과 기존 버전의 블록 ID·번역·검수 이력·메모·북마크·읽기 위치를 유지하며, 새 블록은 새 캐시로 구분한다. 제공자 등록·전환이나 reader 방문만으로 기존 PDF를 재추출하지 않는다. 직접 `extractPdf(bytes)`를 호출하면 기존 `structured-v2`를 사용하는 호환 계약과 상세 저장 분기는 SYSTEM_DESIGN.md 8.1을 따른다.

제목·슬라이드 번호만 텍스트로 추출되고 본문이 이미지인 페이지는 문단 병합으로 복구되지 않는다. 현재 글자 수 기반 검사만으로 이런 페이지를 모두 식별한다고 보장하지 않으며, OCR·복잡한 표 추출은 후속 범위로 남긴다. 추출 상태 `ready`를 모든 본문·수식·도표의 완전 추출이나 번역 품질 보장으로 표시하지 않는다.
...
번역 버튼을 누른 직후부터 실제 대기·번역 작업이 끝날 때까지 원형 회전 표시와 한국어 상태 문구를 제공한다. 버튼 요청이 접수됐다는 이유만으로 로딩을 끝내지 않는다. 완료·실패·취소 시 해제하며 모바일에서도 지속 상태를 확인할 수 있게 한다. 동작 줄이기 설정에서는 회전 없이 상태 문구를 유지한다.
...
설정 생략 시 초기 기본 제공자는 Argos다. 이 PC는 별도 Q26 비교·격리 앱 검증 후 Hy-MT2 문맥 구성을 명시 선택해 운영 HTML 3문단의 생성·저장·캐시를 확인했지만, 2026-09-20 CPU 긴 문단의 지연과 절전 복귀 실패 때문에 현재 웹 운영 제공자를 Argos로 되돌렸다. Hy-MT2의 운영 metadata `html-only` 이력과 PDF 격리 앱 검증, 모델·등록·기존 번역은 보존한다. 학습·평가를 통과한 결과라도 배포 형식 변환 후 추론 비교·무결성 확인을 마치기 전에는 앱 적용 완료로 보고하지 않는다. `.training/`의 개인 파생 가중치와 실행 기록은 DB 백업에 포함되지 않으므로 별도로 보관한다. 구체적인 명령·모델 출처·현재 실행 상태는 [학습 도구 안내](scripts/model-training/README.md)와 [구현 상태](IMPLEMENTATION_STATUS.md)를 따른다.

2026-09-09 추가 요청으로 새 기반 모델의 로컬 실행·품질·속도를 별도 탐색한다. [비교 자료](content/model-comparison/README.md)는 기존 train/dev/test를 재사용하지 않은 도우미 작성 미검수 12개 문단이며, 독립 최종 시험이나 앱 등록 기준이 아니다. Argos·기존 FP32 학습 모델·TranslateGemma 4B의 공개 제3자 Q4_K_M 변환본을 구분하고, 원시 모델의 용어·수식·부정·누락과 실행 장치·정밀도·시간을 기록한다. 이 비교는 추가 가중치 학습이나 기본 제공자 변경을 수행하지 않는다.

추가 로컬 모델 비교에서 사용자는 번역 속도보다 학습자가 읽는 한국어의 의미 정확성과 자연스러움을 우선한다. [언어학적 비교 원칙](content/model-comparison/LINGUISTIC_OPTIMIZATION_PROTOCOL.md)에 따라 다의어·주체와 수취 관계·조건·부정·수량 관계를 별도로 검토한다. 비교용 모델은 순차 로드하고 끝난 실행의 메모리를 반환한다. 작은 개발 자료의 용어 일치율이나 실행 속도를 전체 번역 품질 보증으로 표시하지 않는다.

### 7.9 자동 의미 위험 표시 — 2026-09-11 추가 승인

사용자는 이후 번역·밑줄 검사 모두 95% 목표를 제시한 뒤 언어학·교육학 근거로 최종 baseline을 정하도록 위임했다. [금융자료 학습 준비 기준](content/model-comparison/LEARNING_READINESS_BASELINE_20260911.md)을 새 수용 정책으로 적용한다. 핵심/중요 의미 오류는 평가 표본에서 0건이어야 하며 용어·질문·위험 경고 지표를 별도 분모로 확인한다. 과거 실험의 사전 기준·결과를 소급 변경하거나 자동 점수를 인간 학습 효과로 해석하지 않는다.

추가 사용자 지시에 따라 개발 비교의 오류 근거를 [통합 원장](scripts/model-comparison/error-ledger/README.md)에 누적한다. 원문·번역·문맥 정체성과 검토회차를 보존하며 번역 오류, 질문 오답, 검사기의 오탐/미탐, 실행 실패·사용자 취소를 구분한다. 관측과 원인 가설을 분리하고, 같은 출력의 재검토를 새 오류로 합산하지 않는다. 이 기록은 운영 검수나 학습 데이터로 자동 편입하지 않으며 독립 test를 조정용 원장에 넣지 않는다.

실패 재시도에서 이미 완료한 문단을 보존하고 미완료 문단만 별도로 실행할 수 있다. 이때 선택 실행과 원래 실패의 범위를 명시하고, 서로 다른 실행의 결과를 하나의 전체 성공 실행으로 표시하지 않는다. 전체 의미·용어·질문 수용 기준과 사람/도우미 검토의 구분은 유지한다.

2026-09-13 후속 개선의 우선순위는 [인수인계28절](TRANSLATION_HANDOFF_20260911.md#28-공통-입력-처리와-의미-관계-검사-개선-실행-계획)을 따른다. 구조를 반영한 문맥 선택·문맥별 용어 사전으로 오역 발생을 줄이고, 별도 수량/논리 관계 검사로 검출력을 개선한다. 같은 Hy7 가중치·생성 조건에서 현재 구성/문맥 개선/사전 개선/결합 구성을 분리 비교하며 일반 문장 회귀·원문 의미·독립 질문을 확인한다. 입력에는 원문과 출처 있는 일반 사전/허용 문맥만 사용하고 평가 정답을 넣지 않는다. TG27 미완료 읽기는 후순위로 보존한다. 현재는 실행 계획이며 새 기능·모델 품질·앱 적용 완료를 뜻하지 않는다. 기존 수용 기준·등록·개인 기록·동결 평가를 보존한다.

2026-09-13 S1의 [입력·평가 계약](content/model-comparison/input-preparation-v1/CONTRACT.md)을 확정했다. 금융8/일반8 개발16단위·48핵심명제·32질문(핵심16)·39선정용어와 출처 있는8개념 사전을 추론 전에 고정한다. 공개4단위는 기존 train과 중복하는 회귀이며 새 독립 원문이 아니다. C0의 실제 대상 중복·문자 제한을 기준으로 보존하고 새 사전은 금융 뜻 힌트와 비금융 오적용 억제를 구분한다. 후속 독립 금융60/일반20은 후보 동결 뒤 새 문서 그룹에서 선정하며 그 내용은 개발에 사용하지 않는다.

후속 [S2/S3](content/model-comparison/input-preparation-v1-s2s3/README.md)는 운영 경로 밖에서 입력 선택·64개 실제 tokenized prompt·기존98출력 관계 진단을 완료했다. 이번 개발16의 새 금융 힌트0·기존 오류 재검출1을 그대로 기록하며 의미 개선으로 해석하지 않는다. 예산 조정은 문맥/의존 힌트 제거만 허용하고 제거 뒤 새 뜻 힌트를 추가하지 않는다. 미지원/보류는 통과가 아니며 원장 관측 추가는 새 번역 오류 증가와 구분한다.

사용자의 후속 지시로 [S4부터 조건부 S6까지](content/model-comparison/input-execution-v1/README.md) 진행한다. 최초 실행은 실제64개 입력 일치 후 38개를 저장했으나 전원 조건으로 중단됐다. [기술 복구](content/model-comparison/input-execution-v1/RECOVERY_V1.md)는 원래 실패·미완료와 완료 출력38개를 보존하고 나머지26개만 같은 고정 입력/설정으로 새 실행에 생성한다. AC 등 기존 조건을 완화하거나 품질을 보고 입력·사전·샘플러를 바꾸지 않는다. 별도 논리 묶음의64개 완결성과 두 실행의 최종 무결성을 확인한 뒤 전체 원문과 독립 한국어 질문을 평가한다. 새 중요 오류·핵심 명제 손상·일반 회귀가 없는 의미 개선 후보만 선택·동결하고 독립 확대 평가로 진행한다. 적격 후보가 없으면 기존 등록 유지로 S6을 결론 내린다. 개발 성공만으로 앱 적용·학습 수용 기준 충족을 표시하지 않는다.

S1이 허용한 동등 격리 답변자의 구체 경로로 [로컬 독립 질문 절차 v1](content/model-comparison/input-execution-v1/LOCAL_QUESTION_REVIEW_PROTOCOL_V1.md)을 사용한다. 새 에이전트 생성 한도를 실제 spawn 성공으로 기록하지 않는다. 전체64개에 같은 설치된 Qwen 모델과 설정을 사용하고 단위×구성마다 새 native 프로세스·단일 completion·이전 cache0을 검증한다. 한국어 번역과 질문 외의 원문·정답·구성·다른 출력은 모델에 전달하지 않으며, 전체 답변을 동결한 뒤 별도 원문 채점자가 채점한다. 모델 답변의 의미를 root가 보완하거나 사람 검수·일반 정확도 인증으로 표시하지 않는다. 실제 설치 목록 오류는 원래 동결을 보존하는 [별도 교정 계약](content/model-comparison/input-execution-v1/LOCAL_QUESTION_INVENTORY_CORRECTION_V2.md)으로 처리했다. [현재 전원 중단 기록](content/model-comparison/input-execution-v1/QUESTION_POWER_STOP_20260914.md)의 native/질문 호출0회 상태는 S5 완료나 후보 탈락 판정이 아니다. 이전 AC 전용 계약은 해당 실행의 이력으로 보존한다. 사용자는 2026-09-14 후속 지시로 충전기 없이 배터리 전원에서 계속 실행하도록 명시했다. [배터리 실행 v4](content/model-comparison/input-execution-v1/LOCAL_QUESTION_BATTERY_EXECUTION_V4.md)는 AC0/1과 알려진 배터리 잔량20% 초과를 허용하며 CPU4·메모리·소유 프로세스·시간 제한과 질문/평가 기준을 유지한다. 이전 AC 전용 실패·동결을 보존하고 새 실행 정체성으로 진행한다. 평가 분모와 품질 기준은 완화하지 않는다. [0회 호출 복구 v3](content/model-comparison/input-execution-v1/LOCAL_QUESTION_ZERO_CALL_RECOVERY_V3.md)는 이전 실패3파일·원래 claim·동결 해시를 정확히 검증하는 별도 경로다. 현재 사전조건을 충족하기 전에는 새 run/claim을 만들지 않고, 실제64개 답변 완료·동결 전 채점/후보 선택을 수행하지 않는다. 후속 [질문 부분 복구 v5](content/model-comparison/input-execution-v1/LOCAL_QUESTION_PARTIAL_RECOVERY_V5.md)는 완료9개의 실제 답변을 재생성하지 않고 원래 failed9 실행과 새55개 실행을 별도 정체성으로 연결한다. native 생성 전 시작 메모리만 부족하면 원래9GiB 기준의 두 관측이 통과할 때까지 최대300초/5초 간격으로 대기하며, 배터리·경합 오류나 실제 context 실패는 숨기거나 자동 재호출하지 않는다. 실제 context producer는 동결 v4로 유지하고 coordinator v5 및 sourceRuns/partialRecovery를 별도로 기록한다. 원64개 전체 답변 동결 전 채점하지 않으며 질문·품질 기준과 앱 등록은 유지한다.

2026-09-20 현재 자원으로 시작하려는 사용자 요청에 따라 [메모리 시작 정책 v6](content/model-comparison/input-execution-v1/LOCAL_QUESTION_MEMORY_EXECUTION_V6.md)를 추가한다. 과거 peak 약5.246GiB의 근거로 시작 physical만9→7GiB로 조정하고 commit9GiB·모델 관측 상한6GiB·실행 중 여유1GiB·기존 중단 조건은 유지한다. 원9개는 v4, 새55개는 새 native producer v6로 명시하고 과거 코드·동결·실행을 보존한다. 새 실행/정체성·혼합 producer 검증·수집/동결/채점 연결을 확인한 후 실행하며, 모델·질문·생성 설정과 품질 기준은 변경하지 않는다. 이는 자원 부족으로 중단될 수 있는 새 실행 정책이며 완료나 품질 통과를 보장하지 않는다.

선택해 새로 생성한 번역을 저장한 뒤 별도 비동기 의미 검사 작업을 등록한다. 기존 번역은 사용자가 `의미 검사`를 요청할 때만 검사하며, 페이지 방문·렌더링·조회는 평가를 시작하지 않는다. 숫자·수식 무결성, 의미 위험, 사용자 검수, 최신 여부를 별도로 표시한다. 점수는 정확도 확률이 아니며 기본 화면에는 위험 단계와 근거를 제공한다.

- 검증한 고정 규칙 또는 신뢰 기준을 통과한 중요/심각 오류 구간은 한국어의 유효한 UTF-16 offset·문자열을 확인한 뒤 빨간 물결 밑줄로 표시한다. 영어는 연결된 문장 전체를 강조한다. 정밀한 양언어 구절 정렬을 구현했다고 표시하지 않는다.
- 문단 점수만 낮으면 문단 전체를 옅은 주황색으로 표시하고 `자동 의미 검사: 검토 권장`을 안내한다. 오류 단어를 임의로 특정하지 않는다. 색 외에 텍스트·아이콘·근거 보기·번역 수정 동선을 제공한다.
- 대기/실행/완료/미설치·미등록/실패/취소/이전 설정 결과를 구분한다. `자동 검사상 특이점 없음`도 사용자 미검수다. 평가 실패 시 번역을 보존하고, 제한된 규칙에서 확인한 경고는 별도로 보여준다. 표 의미 검사는 초기 범위에서 제외한다.
- 기존 48개 고정 출력·도우미 판정을 유지한 채 참조 번역 없이 평가하는 로컬 후보를 실측한다. 오류 재현율·경고 정밀도·전체 경고율·유형별 누락·길이·모델별 분포·시간·메모리를 기록한다. 임계값과 등록 기준을 버전으로 보존하고 실패 기준을 사후 완화하지 않는다. 작은 개발 표본의 교정을 독립 최종 인증으로 표현하지 않는다.
- 원문·번역·문맥 해시, 평가 모델·가중치·실행 코드·교정·규칙 정체성을 평가 이력과 캐시에 포함한다. 기계 번역의 validation_status를 의미 점수로 덮어쓰지 않는다. 검수 저장 후에는 검수본을 우선하고 늦은 자동 평가로 바꾸지 않는다.
- 번역 배치를 먼저 처리하고 소유 번역 프로세스의 종료를 확인한 뒤 평가 배치를 시작한다. 별도 `.venv-qe/`와 `.translation/qe/`를 사용하고 검증된 등록이 없으면 대체 모델로 조용히 전환하지 않는다.
...
- 같은 자료·파일 해시·추출기·추출 설정이면 중복 원문 버전을 만들지 않는다. 추출 규칙이 바뀌면 별도 버전으로 저장하고 기존 블록과 그에 연결된 번역·개인 기록을 보존한다.
...
- 실제 번역 엔진 검증: 현재 선택한 로컬 모델로 번역·저장·캐시를 확인한 결과. 기본 Argos와 등록 후 명시 선택한 Hy-MT2 등은 제공자·모델 정체성을 구분해 기록. 선택형 외부 API 실호출은 별도 기록하며 이번 무료 로컬 개선의 검증에 사용하지 않음.
...
PDF 전용 추출 개선은 신규 12개·기존 관련 3개 테스트와 TypeScript 검사를 통과했다. 검증에는 숫자 조각·글머리·회전·표 라벨, 격리 DB에서의 새 버전 저장과 기존 블록·번역·검수·개인 기록 보존, 실제 `session2.pdf` 3페이지의 13줄을 제목·본문·슬라이드 번호 7블록으로 묶는 검사가 포함된다. 별도 시각 대조에서 PDF 7·9페이지의 표·회전 라벨을 확인하고 병합 보류와 원문 보존도 재현했다. 이는 추출·저장 검증이며 실제 로컬 모델의 번역·저장·캐시 검증 완료와는 별도로 기록한다. 현재 실행 근거와 남은 제약은 [구현 상태](IMPLEMENTATION_STATUS.md)를 따른다.
