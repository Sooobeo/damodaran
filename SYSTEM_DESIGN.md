...
2026-09-20 [메모리 시작 정책 v6](content/model-comparison/input-execution-v1/LOCAL_QUESTION_MEMORY_EXECUTION_V6.md)는 현재 자원으로 시작하려는 사용자 요청을 반영한 별도 native producer/coordinator다. PROFILE의 시작 physical만9→7GiB로 바꾸며 commit9GiB·관측 상한6GiB·실행 중 여유1GiB와 다른 보호를 유지한다. `partialRecovery.previousPartialRecovery`에 동결 v5를 연결하고 originalProfile/profile/memoryPolicy를 기록한다. 원9개는 v4·9GiB, 새55개는 v6·7GiB로 각각 검증하며 sourceRuns·각 context의 producer를 수집/답변 동결/평가/원장까지 보존한다. v5 미실행 진입점은 명시 차단 표시로 중복 생성을 막고 실제 v5 호출로 세지 않는다. 질문·정답 격리·모델/생성 설정·전체64 동결 전 채점 금지·기존 품질 기준은 유지한다.
